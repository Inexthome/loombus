-- Issue #674: separate legal-request review authority from intake authority.
-- This migration does not enable the capability for any operator and does not
-- enable counsel approval, export generation, disclosure, notice sending, or transmission.

begin;

alter table public.legal_operations_authorizations
  add column if not exists can_review_requests boolean not null default false;

comment on column public.legal_operations_authorizations.can_review_requests is
'Allows restricted legal-request review metadata changes including identity, authority, scope, deficiency/rejection, cross-border, confidentiality, notice-decision metadata, and transparency classification. Does not authorize counsel approval, export, disclosure, emergency approval, notice sending, or external transmission.';
...
-- The trigger is the authoritative boundary for every service-side UPDATE path,
-- including the existing Legal Operations API. Intake-only operators may still
-- create request records, but cannot mutate review metadata after creation.
drop trigger if exists legal_requests_enforce_review_authorization on public.legal_requests;
create trigger legal_requests_enforce_review_authorization
before update on public.legal_requests
for each row execute function public.legal_enforce_request_review_authorization();
