-- Library paid-book tax audit and idempotency state.
--
-- New purchases snapshot the checkout tax posture, calculated tax, and any
-- Stripe transfer reversal used to withhold platform-liable tax from a
-- destination charge. Historical purchases remain valid with null tax fields.
...
create unique index if not exists library_book_purchases_tax_transfer_reversal_uidx
...
comment on column public.library_book_purchases.tax_amount_cents is
  'Indirect tax calculated by Stripe for the completed checkout. Zero when no Stripe tax was collected.';
