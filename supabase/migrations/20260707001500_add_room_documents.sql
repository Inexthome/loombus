create table if not exists public.room_documents (
  ...
  file_url text not null check (char_length(file_url) between 1 and 1000),
...
create index if not exists room_documents_room_idx on public.room_documents (room_id, is_pinned desc, category, title);
create index if not exists room_documents_created_by_idx on public.room_documents (created_by);
create index if not exists room_documents_category_idx on public.room_documents (category);

alter table public.room_documents enable row level security;

create or replace function public.user_can_access_room_documents(target_room_id uuid)
returns boolean
language sql
security definer
...
create or replace function public.user_can_manage_room_documents(target_room_id uuid)
returns boolean
language sql
security definer
set search_path = 'public'
...
drop policy if exists "Room members can view documents" on public.room_documents;
create policy "Room members can view documents"
  on public.room_documents
  for select
  using (public.user_can_access_room_documents(room_id));

drop policy if exists "Room owners and admins can create documents" on public.room_documents;
create policy "Room owners and admins can create documents"
  on public.room_documents
  for insert
  with check (
    created_by = auth.uid()
    and public.user_can_manage_room_documents(room_id)
  );

drop policy if exists "Room owners and admins can update documents" on public.room_documents;
create policy "Room owners and admins can update documents"
  on public.room_documents
  for update
  using (public.user_can_manage_room_documents(room_id))
  with check (public.user_can_manage_room_documents(room_id));

drop policy if exists "Room owners and admins can delete documents" on public.room_documents;
create policy "Room owners and admins can delete documents"
  on public.room_documents
  for delete
  using (public.user_can_manage_room_documents(room_id));
...
as $function$
begin
  if source_table = 'room_documents' and source_operation = 'INSERT' then
...
begin
  if to_regprocedure('public.install_room_activity_trigger(regclass)') is not null then
    perform public.install_room_activity_trigger('public.room_documents'::regclass);
  end if;
end $$;
