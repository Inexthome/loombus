...
create table if not exists public.room_form_submissions (
  id uuid primary key default gen_random_uuid(),
  room_id uuid not null references public.rooms(id) on delete cascade,
  form_id uuid not null references public.room_forms(id) on delete cascade,
...
create index if not exists room_forms_room_idx on public.room_forms (room_id, category, title);
create index if not exists room_forms_created_by_idx on public.room_forms (created_by);
create index if not exists room_forms_category_idx on public.room_forms (category);
create index if not exists room_form_submissions_room_idx on public.room_form_submissions (room_id, status, updated_at desc);
create index if not exists room_form_submissions_form_idx on public.room_form_submissions (form_id, status, updated_at desc);
create index if not exists room_form_submissions_submitted_by_idx on public.room_form_submissions (submitted_by);
...
create or replace function public.user_can_access_room_forms(target_room_id uuid)
returns boolean
language sql
security definer
...
create or replace function public.user_can_manage_room_forms(target_room_id uuid)
returns boolean
language sql
security definer
set search_path = 'public'
...
drop policy if exists "Room members can view forms" on public.room_forms;
create policy "Room members can view forms"
  on public.room_forms
  for select
  using (public.user_can_access_room_forms(room_id));

drop policy if exists "Room owners and admins can create forms" on public.room_forms;
create policy "Room owners and admins can create forms"
  on public.room_forms
  for insert
  with check (
    created_by = auth.uid()
    and public.user_can_manage_room_forms(room_id)
  );

drop policy if exists "Room owners and admins can update forms" on public.room_forms;
create policy "Room owners and admins can update forms"
  on public.room_forms
  for update
  using (public.user_can_manage_room_forms(room_id))
  with check (public.user_can_manage_room_forms(room_id));

drop policy if exists "Room owners and admins can delete forms" on public.room_forms;
create policy "Room owners and admins can delete forms"
  on public.room_forms
  for delete
  using (public.user_can_manage_room_forms(room_id));

drop policy if exists "Room members can view own submissions" on public.room_form_submissions;
create policy "Room members can view own submissions"
  on public.room_form_submissions
  for select
  using (
    public.user_can_manage_room_forms(room_id)
    or (
      submitted_by = auth.uid()
      and public.user_can_access_room_forms(room_id)
    )
  );

drop policy if exists "Room members can create submissions" on public.room_form_submissions;
create policy "Room members can create submissions"
  on public.room_form_submissions
  for insert
  with check (
    submitted_by = auth.uid()
...
drop policy if exists "Room owners and admins can update submissions" on public.room_form_submissions;
create policy "Room owners and admins can update submissions"
  on public.room_form_submissions
  for update
  using (public.user_can_manage_room_forms(room_id))
  with check (public.user_can_manage_room_forms(room_id));

drop policy if exists "Room owners and admins can delete submissions" on public.room_form_submissions;
create policy "Room owners and admins can delete submissions"
  on public.room_form_submissions
  for delete
  using (public.user_can_manage_room_forms(room_id));
...
as $function$
begin
  if source_table = 'room_forms' and source_operation = 'INSERT' then
    return 'Form created';
  elsif source_table = 'room_forms' and source_operation = 'UPDATE' then
    return 'Form updated';
  elsif source_table = 'room_forms' and source_operation = 'DELETE' then
    return 'Form removed';
  elsif source_table = 'room_form_submissions' and source_operation = 'INSERT' then
    return 'Form submission received';
  elsif source_table = 'room_form_submissions' and source_operation = 'UPDATE' then
    return 'Form submission ' || coalesce(row_data ->> 'status', 'updated');
  elsif source_table = 'room_form_submissions' and source_operation = 'DELETE' then
    return 'Form submission removed';
  elsif source_table = 'room_documents' and source_operation = 'INSERT' then
    return 'Document added';
  elsif source_table = 'room_documents' and source_operation = 'UPDATE' then
    return 'Document updated';
  elsif source_table = 'room_documents' and source_operation = 'DELETE' then
    return 'Document removed';
  elsif source_table = 'room_faq_entries' and source_operation = 'INSERT' then
...
begin
  if to_regprocedure('public.install_room_activity_trigger(regclass)') is not null then
    perform public.install_room_activity_trigger('public.room_forms'::regclass);
    perform public.install_room_activity_trigger('public.room_form_submissions'::regclass);
  end if;
end $$;
