...
create table if not exists public.room_activity_log (
  ...
  title text not null,
  created_at timestamptz not null default now(),
  constraint room_activity_log_activity_type_check
    check (activity_type in ('room_discussion', 'room_announcement', 'room_event')),
  constraint room_activity_log_target_type_check
    check (target_type in ('room_post', 'room_announcement', 'room_event')),
  constraint room_activity_log_title_length_check
    check (char_length(title) between 1 and 200)
);

create unique index if not exists room_activity_log_target_unique_idx
  on public.room_activity_log (target_type, target_id);
create index if not exists room_activity_log_room_created_idx
  on public.room_activity_log (room_id, created_at desc);
...
grant select, insert, update, delete
  on public.room_notification_preferences to authenticated;
revoke all on public.room_activity_log from anon, authenticated;
...
grant execute on function public.room_notification_access_allowed(uuid, uuid)
  to authenticated;
...
begin
  ...
  if tg_table_name = 'room_posts' then
    ...
    v_activity_type := 'room_discussion';
    v_target_type := 'room_post';
    v_target_id := new.id;
    v_activity_title := left(coalesce(nullif(btrim(new.title), ''), 'New Room discussion'), 160);
    v_preference_kind := 'new_discussions';
  elsif tg_table_name = 'room_announcements' then
    v_actor_id := new.created_by;
    v_activity_type := 'room_announcement';
    v_target_type := 'room_announcement';
    v_target_id := new.id;
    v_activity_title := left(coalesce(nullif(btrim(new.title), ''), 'Room announcement'), 160);
    v_preference_kind := 'announcements';
  elsif tg_table_name = 'room_events' then
    v_actor_id := new.created_by;
    v_activity_type := 'room_event';
    v_target_type := 'room_event';
    v_target_id := new.id;
    v_activity_title := left(coalesce(nullif(btrim(new.title), ''), 'Room event'), 180);
  ...
  select
    ...
    v_target_id,
    new.room_id,
    case v_preference_kind
      when 'new_discussions' then
        'New discussion in ' || v_room_name || ': ' || v_activity_title
      when 'announcements' then
        'New announcement in ' || v_room_name || ': ' || v_activity_title
      else
        'New event in ' || v_room_name || ': ' || v_activity_title
    end
  from (
  ...
  where recipient.user_id is distinct from v_actor_id
...
drop trigger if exists room_posts_create_activity_notifications
...
drop trigger if exists room_events_create_activity_notifications
