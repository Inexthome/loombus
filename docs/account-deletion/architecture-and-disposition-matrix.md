...
| Private-message conversations and content | `private_conversations`, `private_conversation_members`, `private_messages` | “Delete conversation” sets only the requesting member's `deleted_at`; shared content remains readable to the other participant and administrators. Auth deletion currently cascades through member and sender foreign keys. | Preserve the shared thread for the remaining participant pending an approved retention schedule; revoke the departing member's access near finalization; anonymize attribution only after Auth, profile, recipient-access, and evidence prerequisites clear | reports, safety, recipient continuity, legal hold | migration `20260804008000`, private-message schema and APIs | Disposition defined; manual-review only; Auth cascade must be corrected before Auth deletion |
| Private-message attachments and evidence | `private_message_attachments`, `message-attachments` Storage objects and URLs, message reports, trust-and-safety evidence | Attachment metadata cascades from messages and conversations; clients store and render public URLs; reports can preserve message or conversation context | Handle separately through a verified Storage manifest and evidence/legal-hold decision; never inherit a database cascade as proof of object deletion | reports, safety, recipient access, vendor copies, legal hold | migration `20260804008000`, attachment upload and administrator evidence APIs | Separate manual-review resource; bucket privacy, evidence coverage, and object deletion are unverified |
| Rooms, organizations, and shared Room data | Room tables, organizations, memberships, posts, replies, files, calendars, governance, billing references, audit, and lifecycle records | Room ownership can be transferred and permanent deletion has an independent recovery, preflight, Storage-manifest, and reconciliation workflow. Several Auth foreign keys still cascade through shared Room records. | Transfer ownership and then revoke the departing member when an approved successor exists; otherwise use the independent staged Room deletion lifecycle after all gates pass | recipient continuity, organization ownership, billing, reports, retention, legal hold | migration `20260804009000`, Room governance and deletion-state-machine migrations | Disposition defined; manual-review only; transfer contract and Auth cascades must be reconciled before Auth deletion |
| Commerce and local records | Marketplace, Businesses, Services, Requests, Jobs, Events, Appointments, Local | Public and shared records can involve owners, authors, providers, customers, applicants, employers, organizers, attendees, counterparties, billing, disputes, moderation, Storage, search, and vendor copies | Decide delete, anonymize, retain, or transfer per table and record only after ownership, participant continuity, billing, evidence, Storage, search, vendor, and Auth prerequisites clear | transaction dispute, fraud, billing, tax, accounting, moderation, safety, legal hold, counterpart continuity | migration `20260804233000` and module migrations | Disposition defined; manual-review only; no commerce or local mutation handler approved |
...
## Private messages and attachments disposition

Migration `20260804008000_account_deletion_private_messages_disposition.sql` records the private-message policy without adding a handler or worker dispatch.

- A conversation is shared with another participant. The current member-facing delete action only hides the conversation for that member by setting `private_conversation_members.deleted_at`; it does not erase the shared thread.
- Conversation containers, remaining-participant access, message bodies, stable IDs, timestamps, and thread relationships remain preserved pending an approved retention schedule and evidence review.
- The departing member's access is revoked only in the approved finalization sequence. The other participant's membership and access cannot be removed by the account request.
- Public attribution is removed later through the approved profile-anonymization path. Message and participant rows are not rewritten by this phase.
- `private_conversation_members.user_id` and `private_messages.sender_id` currently cascade from `auth.users`. Supabase Auth deletion must remain blocked until those cascades are replaced with an approved attribution or tombstone strategy.
- Attachment metadata, `message-attachments` Storage objects, public URLs, reports, and trust-and-safety evidence remain separate manual-review resources. Database row deletion is not proof that a Storage object or vendor copy was deleted.
- An account request cannot automatically delete, rewrite, anonymize, or remove access to private-message resources under this phase.

## Rooms ownership and staged deletion disposition

Migration `20260804009000_account_deletion_room_disposition.sql` records the Room policy without adding an account-deletion handler, ownership mutation, access mutation, or Room deletion call.

- A member account request never directly deletes a Room. If the Room has an approved successor or continuing community, business, support, classroom, HOA, or organization purpose, ownership must be transferred and verified first.
- The current transfer contracts are not interchangeable. `transfer_room_ownership` blocks active paid billing and rewrites Room owner, creator, membership, and local Stripe references. `accept_room_ownership_transfer` changes `owner_id` and membership roles but does not reconcile `created_by`, organization ownership, or billing references.
- An accepted governance transfer leaves the former owner as an administrator. The departing member's Room role and access must be explicitly revoked only after ownership, organization, and billing continuity are durable.
- If no approved successor or continuing purpose remains, Room deletion must use the independent Room lifecycle: archive, recovery period, current inactive-billing verification, retention and legal-hold checks, persisted Storage manifest, object reconciliation, and final deletion job.
- Auth deletion remains blocked because Room membership, posts, replies, events, resources, organizations, transfers, moderation reports, and other shared participation records include cascading member foreign keys.
- Room Storage objects, Stripe and other vendor copies, search and cache entries, AI derivatives, moderation evidence, backups, replicas, and Floor research rooms remain separate resources.
- `ROOM_PERMANENT_DELETION_ENABLED` and `ACCOUNT_DELETION_DESTRUCTIVE_HANDLERS_ENABLED` are independent gates. This phase changes neither flag and cannot transfer or delete a Room.

## Commerce and local records disposition

Migration `20260804233000_account_deletion_commerce_records_disposition.sql` records the policy for Marketplace, Businesses, Services, Requests, Jobs, Events, Appointments, and Local without adding a mutation handler or worker dispatch.

- The private save tables remain governed by the separately gated commerce-saves handler. This phase governs authored, owned, shared, transactional, participant-linked, or operational commerce and local records.
- Every relevant table and record must receive an approved delete, anonymize, retain, or transfer decision. A module name alone is not sufficient evidence for mutation.
- Public listings, business and provider continuity, applications, bookings, appointments, event participation, counterpart access, stable IDs, status history, billing, disputes, fraud, moderation, safety, support, audit, and legal-hold evidence remain preserved until their prerequisites clear.
- Storage objects and URLs, Stripe and other billing records, search documents and caches, notifications and delivery-vendor copies, AI derivatives and provider copies, analytics, logs, backups, and replicas remain separate resources.
- Supabase Auth deletion and profile anonymization remain blocked until commerce and local ownership, attribution, participant continuity, cascades, and evidence requirements are reconciled.
- This phase does not delete, anonymize, transfer, unpublish, close, or otherwise modify a commerce or local record.
- `ACCOUNT_DELETION_DESTRUCTIVE_HANDLERS_ENABLED` is unchanged and does not authorize commerce or local record mutation.

## Processor rollout state

The processor is now fail-closed. It claims work safely, verifies that the account remains restricted, creates a disposition for every enabled registry resource, and writes a structured exception report. Administrators can record evidence-backed outcomes for manual or external resources and requeue a reviewed request; automatic handlers cannot be manually overridden. Reviewed outcomes and completed irreversible automatic outcomes remain durable across retries. Once an outcome is marked irreversible, cancellation and account restoration are blocked at the database layer. The first-party notification, private-personalization, private-activity, private-goals, private-matching-configuration, private-Floor-cloud, private-Discussion-audience-preferences, member-product-feedback, and private-commerce-saves handlers are implemented but require the explicit `ACCOUNT_DELETION_DESTRUCTIVE_HANDLERS_ENABLED=true` runtime gate, so deployment alone cannot activate them. Profile, public-content, private-message, Room, and commerce/local-record dispositions are now defined as manual-review resources without handlers. Auth deletion, vendor deletion, public-content erasure, private-message mutation, profile anonymization, Room ownership mutation, Room deletion, and commerce/local record mutation remain disabled until their prerequisites are approved and their handlers are separately verified. Requests with any unresolved resource move to `blocked`, never `completed`.
