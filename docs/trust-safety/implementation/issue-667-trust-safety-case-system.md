...
The migration creates three service-only tables:
...
3. Run `scripts/verification/trust-safety-case-system-readiness.sql` and require every row to return `PASS`.
4. Sign in as an authorized administrator and open `/admin/reports/trust-safety`.
5. Create a fictional S4 test case without real member data or harmful material.
6. Confirm a generated case number, case-created event, and audit record.
7. Update severity, status, factual fields, and decision fields; confirm append-only change events.
8. Add a fictional evidence reference containing only a synthetic record ID and synthetic hash.
9. Add a handling event and confirm it cannot be edited or deleted through the database contract.
10. Reopen the case and confirm an authorized access event is added.
11. Close the fictional case and confirm `closed_at`, `closed_by`, and the closure event.
12. Delete no production evidence. Retain or remove the synthetic test record only under an approved test-data procedure.
