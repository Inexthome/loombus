import fs from "node:fs";

const routes = [
  ["reports", "ReportsV2Client", "reports-v2-page"],
  ["safety", "SafetyV2Client", "safety-v2-page"],
  ["enforcement", "AdminEnforcementClient", "admin-enforcement-page"],
  ["deleted", "AdminDeletedV2Client", "deleted-v2-page"],
  ["deleted-replies", "AdminDeletedRepliesV2Client", "deleted-replies-v2-page"],
  ["audit", "AdminAuditV2Client", "audit-v2-page"],
];

const sharedPath = "src/app/admin/trust-safety-editorial.css";
const nativePath = "src/app/admin/admin-trust-safety-native.css";
const framePath = "src/app/admin/admin-trust-safety-editorial-frame.tsx";
const shared = fs.readFileSync(sharedPath, "utf8");
const native = fs.readFileSync(nativePath, "utf8");
const frame = fs.readFileSync(framePath, "utf8");

function requireText(text, value, message) {
  if (!text.includes(value)) throw new Error(message);
}

for (const [route, component, scope] of routes) {
  const pagePath = `src/app/admin/${route}/page.tsx`;
  const page = fs.readFileSync(pagePath, "utf8");
  requireText(page, 'import "../trust-safety-editorial.css"', `${route} must load the shared Admin Editorial layer.`);
  requireText(page, "AdminTrustSafetyEditorialFrame", `${route} must use the native Editorial suite frame.`);
  requireText(page, `active="${route}"`, `${route} must identify its active Editorial section.`);
  requireText(page, component, `${route} client composition changed unexpectedly.`);
  requireText(shared, `.${scope}`, `${route} must be covered by the shared Admin Editorial layer.`);
}

const reportsPage = fs.readFileSync("src/app/admin/reports/page.tsx", "utf8");
requireText(reportsPage, 'href="/admin/reports/trust-safety"', "Restricted Trust and Safety navigation changed unexpectedly.");
requireText(reportsPage, "Restricted Trust & Safety cases", "Restricted Trust and Safety label changed unexpectedly.");

requireText(frame, 'import "./admin-trust-safety-native.css"', "Native Admin Editorial frame must load its structural stylesheet.");
requireText(frame, "admin-trust-safety-editorial-masthead", "Admin suite must have a native Editorial masthead.");
requireText(frame, "admin-trust-safety-editorial-nav", "Admin suite must have native cross-route navigation.");
requireText(frame, 'aria-current={active === item.key ? "page" : undefined}', "Admin suite navigation must expose active-route semantics.");
requireText(native, "admin-trust-safety-editorial-workspace", "Native Admin Editorial workspace structure is missing.");
requireText(native, ".reports-v2-hero-copy", "Reports legacy hero must be demoted beneath the native Editorial frame.");
requireText(native, ".admin-enforcement-hero > div:first-child", "Enforcement legacy hero must be demoted beneath the native Editorial frame.");
requireText(native, "display: none !important", "Legacy duplicate masthead copy must be removed from the rendered hierarchy.");

requireText(shared, "background: var(--loombus-page-bg) !important", "Admin trust and safety routes must use the standard Loombus page background.");
requireText(shared, "border-bottom: 1px solid var(--loombus-border) !important", "Admin trust and safety routes must use divider-led Editorial structure.");
requireText(shared, "#CBAB5B", "Admin trust and safety routes must retain restrained Loombus Gold.");
requireText(shared, "box-shadow: none !important", "Admin trust and safety routes must remove dashboard elevation.");
requireText(shared, "prefers-reduced-motion", "Admin trust and safety routes must preserve reduced-motion behavior.");
requireText(native, "prefers-reduced-motion", "Native Admin Editorial frame must preserve reduced-motion behavior.");

if (/radial-gradient|linear-gradient/.test(shared) || /radial-gradient|linear-gradient/.test(native)) {
  throw new Error("Admin Editorial layers must not introduce decorative gradients.");
}

console.log("Admin Trust and Safety Editorial UI verification passed.");
