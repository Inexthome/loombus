...
for (const path of [pagePath, surfacePath]) {
...
if (!page.includes("LibraryKnowledgeGraphV4Surface")) throw new Error("Graph route must render v4 surface");
for (const table of [
  ...
  if (!surface.includes(`from(\"${table}\")`)) throw new Error(`Missing graph source: ${table}`);
}
for (const token of [
  "canvasRef",
  "beginPan",
  ...
  if (!surface.includes(token)) throw new Error(`Missing Knowledge Graph v4 scale/interaction contract: ${token}`);
}
for (const relation of ["derived from opening post", "derived from reply", "promoted to discussion"]) {
  if (!surface.includes(relation)) throw new Error(`Missing fixed relationship: ${relation}`);
}
...
if (!surface.includes("supabase.auth.getUser()")) throw new Error("Graph must require authenticated member context");
if (!surface.includes("This graph is private and read-only")) throw new Error("Private/read-only boundary must be visible");
for (const forbidden of ["SUPABASE_SERVICE_ROLE", "service_role", "library_publication_sources", "library-publication-originals", "dangerouslySetInnerHTML", ".insert(", ".update(", ".delete(", ".upsert("]) {
  if (surface.includes(forbidden)) throw new Error(`Forbidden Knowledge Graph v4 token: ${forbidden}`);
}
