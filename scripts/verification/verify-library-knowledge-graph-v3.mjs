...
for (const token of ["<svg", "<line", "<rect", "markerEnd", "viewBox", "Zoom in", "Zoom out", "Topology inspector", "Trace provenance"]) {
  if (!surface.includes(token)) throw new Error(`Missing visual topology contract: ${token}`);
}
for (const relation of ["derived from opening post", "derived from reply", "promoted to discussion"]) {
  if (!surface.includes(relation)) throw new Error(`Missing fixed relation: ${relation}`);
}
...
for (const forbidden of ["SUPABASE_SERVICE_ROLE", "service_role", "library_publication_sources", "library-publication-originals", "dangerouslySetInnerHTML", ".insert(", ".update(", ".delete(", ".upsert("]) {
  if (surface.includes(forbidden)) throw new Error(`Forbidden graph token: ${forbidden}`);
}
