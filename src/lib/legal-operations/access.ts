...
> {
  ...
  const authorizationResult = await service
    ...
    .select(
      "user_id,role,can_intake,can_preserve,can_export,can_disclose,can_approve_emergency,can_manage_access,active,appointed_by,appointed_at,revoked_at,notes"
    )
    .eq("user_id", user.id)
    .maybeSingle();
