...
type Version = { id: string; publication_id: string; version_number: number; title: string; subtitle: string | null; description: string | null; publication_type: string; author_name: string | null; publisher_name: string | null; language_code: string; isbn: string | null; version_status: string; series_title: string | null; series_position: number | null; edition_label: string | null; subjects: string[]; audience_label: string | null };
type Row = Review & { version: Version };
type Access = "checking" | "allowed" | "denied" | "error";

function seriesLabel(version: Version) {
  if (!version.series_title) return "—";
  return version.series_position ? `${version.series_title} · ${Number(version.series_position).toLocaleString(undefined, { maximumFractionDigits: 2 })}` : version.series_title;
}
...
export default function AdminLibraryRevisionReviewPage() {
  ...
  const loadRows = useCallback(async () => {
    ...
    const versionResult = await supabase.from("library_publication_versions").select("id,publication_id,version_number,title,subtitle,description,publication_type,author_name,publisher_name,language_code,isbn,version_status,series_title,series_position,edition_label,subjects,audience_label").in("id", versionIds);
