    ...
    <div className="mx-auto max-w-7xl px-4 pb-28 pt-8 sm:px-6 lg:px-8 lg:pb-16">
      <header className="border-b border-[var(--loombus-border)] pb-7">
        ...
        <div className="mt-6 grid grid-cols-2 border-y border-[var(--loombus-border)] sm:grid-cols-4">
          {[["Saved", saved.length, Bookmark], ["Folders", collections.length, Folder], ["With notes", privateNoteCount, MessageSquareText], ["Topics", topics.length, Sparkles]].map(([label, count, Icon], index) => <div key={String(label)} className={`flex items-center gap-3 px-3 py-4 sm:px-4 ${index % 2 ? "border-l border-[var(--loombus-border)]" : ""} ${index > 1 ? "border-t border-[var(--loombus-border)] sm:border-t-0 sm:border-l" : ""}`}><Icon className="h-4 w-4 text-[var(--loombus-gold)]" /><div><strong className="block text-xl font-black">{String(count)}</strong><span className="text-xs font-bold text-[var(--loombus-text-muted)]">{String(label)}</span></div></div>)}
        </div>
