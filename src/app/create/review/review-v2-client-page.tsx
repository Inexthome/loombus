...
function parseDraftBody(value: string | null | undefined): ParsedDraft {
  ...
  const metadataLines = firstBlock
    .split("\n")
...
function isValidDiscussionTopic(
  value: string | null | undefined
): value is DiscussionTopic {
  ...
  const checks = [
    {
      ...
      helper: "Use a title long enough to frame the discussion.",
    },
    {
...
export default function CreateReviewV2ClientPage() {
  ...
  const readiness = useMemo(
    () => getReadiness(draft, parsed, tags),
    [draft, parsed, tags]
  );
  const hasDraftContent = Boolean(
    draft?.title?.trim() || parsed.body.trim() || tags.length > 0
  );
  ...
  async function publishDraft() {
    ...
    if (!publishPrepared) {
      setPublishMessage(
        "Complete the required checks and confirm the acknowledgment before publishing."
      );
      return;
    }
    ...
    try {
      ...
      window.location.href = discussionId
        ? `/discussions/${discussionId}`
        : "/discussions";
    } catch {
      setPublishMessage(
        "Unable to publish this draft safely. Your draft was not cleared."
      );
    } finally {
      setPublishing(false);
    }
  ...
  return (
    <main className="min-h-screen bg-[var(--loombus-page-bg)] px-4 pb-24 pt-5 text-[var(--loombus-text)] sm:px-6 lg:px-8">
      <section className="mx-auto max-w-6xl">
        <header className="mb-6 rounded-[2rem] border border-[var(--loombus-border)] bg-[radial-gradient(circle_at_top_left,var(--loombus-cream-soft),transparent_22rem),radial-gradient(circle_at_top_right,var(--loombus-gold-soft),transparent_24rem),var(--loombus-surface)] p-5 shadow-sm sm:p-6">
          <Link
            href="/create"
          ...
          <p className="text-xs font-bold uppercase tracking-[0.24em] text-[var(--loombus-gold-deep)]">
            Loombus Review
          </p>
          ...
          <p className="mt-4 max-w-3xl text-sm leading-6 text-[var(--loombus-text-muted)] sm:text-base">
            Review your autosaved draft, confirm the required checks, and publish it to the live discussion feed. Discussion Purpose remains optional.
          </p>
        </header>

        {message && (
          <div className="mb-5 rounded-2xl border border-[var(--loombus-border)] bg-[var(--loombus-gold-surface)] px-4 py-3 text-sm font-medium text-[var(--loombus-text-muted)]">
            {message}
          </div>
        )}
        {publishMessage && (
            ...
            {!draft || !hasDraftContent ? (
              ...
              <>
                <div className="mb-5 flex flex-wrap items-center justify-between gap-3">
                  <span className="rounded-full border border-[var(--loombus-border)] bg-[var(--loombus-gold-surface)] px-3 py-1 text-xs font-bold uppercase tracking-[0.18em] text-[var(--loombus-gold-deep)]">
                ...
                {parsed.purpose && (
                  <div className="mt-5 rounded-3xl border border-[var(--loombus-border)] bg-[var(--loombus-gold-surface)] p-4 text-sm leading-6 text-[var(--loombus-text-muted)]">
                    <span className="font-bold text-[var(--loombus-text)]">
                      Purpose: {" "}
                    </span>
                    {parsed.purpose}
                  </div>
          ...
          <aside className="space-y-4">
            <section className="rounded-[2rem] border border-[var(--loombus-border)] bg-[var(--loombus-surface)] p-5 shadow-sm">
              <div className="flex items-center gap-3">
                <Send className="size-5 text-[var(--loombus-gold-deep)]" />
                <h2 className="font-black text-[var(--loombus-text)]">
                  Publish preparation
                </h2>
              </div>
              ...
              <p className="mt-3 text-xs leading-5 text-[var(--loombus-text-subtle)]">
                Discussion Purpose, Reality Lens, and Purpose Lane are optional and do not block publishing.
              </p>
              ...
              <button
                ...
                className="mt-4 inline-flex w-full items-center justify-center gap-2 rounded-2xl border border-[var(--loombus-gold)] bg-[var(--loombus-gold)] px-4 py-3 text-sm font-bold text-[var(--loombus-gold-contrast)] shadow-sm transition hover:opacity-90 disabled:cursor-not-allowed disabled:border-[var(--loombus-border)] disabled:bg-[var(--loombus-surface-muted)] disabled:text-[var(--loombus-text-subtle)] disabled:opacity-80"
              >
                {publishing ? (
            ...
            <section className="rounded-[2rem] border border-[var(--loombus-border)] bg-[var(--loombus-surface)] p-5 shadow-sm">
              <div className="flex items-center gap-3">
                <CheckCircle2 className="size-5 text-[var(--loombus-gold-deep)]" />
                <h2 className="font-black text-[var(--loombus-text)]">
                  Readiness summary
                </h2>
              </div>
              <div className="mt-4 h-2 overflow-hidden rounded-full bg-[var(--loombus-surface-muted)]">
                <div
                  className="h-full rounded-full bg-[var(--loombus-gold)]"
                  style={{
                    width: `${Math.round(
            ...
            <section className="rounded-[2rem] border border-[var(--loombus-border)] bg-[var(--loombus-surface)] p-5 shadow-sm">
              <div className="flex items-center gap-3">
                <FileText className="size-5 text-[var(--loombus-gold-deep)]" />
              ...
              <div className="mt-4 flex flex-col gap-3">
                <button
                  ...
                  className="inline-flex appearance-none items-center justify-center gap-2 rounded-2xl border border-[color:color-mix(in_srgb,var(--loombus-gold)_55%,var(--loombus-border))] bg-[var(--loombus-gold-surface)] px-4 py-2 text-sm font-bold text-[var(--loombus-text)] shadow-sm transition hover:border-[var(--loombus-gold)] hover:bg-[var(--loombus-surface-strong)] disabled:cursor-not-allowed disabled:text-[var(--loombus-text-subtle)] disabled:opacity-50"
                >
                  <ClipboardCopy className="size-4 text-[var(--loombus-gold-deep)]" />
                  Copy review draft
                </button>
                <Link
                  href="/create"
