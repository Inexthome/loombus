...
import { BookOpenCheck, Pin, Search } from "lucide-react";
...
function resolveHubAnchor() {
  ...
  return directory ?? polls ?? tasks ?? activity ?? members ?? discussions ?? overview;
}

export function RoomFaqSectionActivator({ roomId }: { roomId: string }) {
  ...
  useEffect(() => {
    ...
    function activate() {
      ...
      const existingHost = document.querySelector<HTMLElement>('[data-room-faq-section-host="true"]');
      if (existingHost) {
      ...
      host.setAttribute("data-room-faq-section-host", "true");
      host.setAttribute("id", "faq");
      ...
      <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        <div className="flex items-start gap-3">
          ...
          <div className="min-w-0">
            <p className="text-xs font-black uppercase tracking-[0.16em] text-amber-700">Room Knowledge Base / FAQ is live</p>
            <h2 className="mt-1 max-w-2xl text-xl font-black leading-tight text-slate-950 sm:text-2xl">Turn repeat questions into room knowledge</h2>
            <p className="mt-2 max-w-2xl text-sm font-semibold leading-6 text-slate-700">
              Store rules, instructions, policies, procedures, and common answers where members can search them instead of asking the same question again.
            </p>
          </div>
        </div>
        <Link href={`/rooms/${roomId}/faq`} className="inline-flex w-full items-center justify-center rounded-2xl bg-slate-950 px-4 py-3 text-sm font-black text-white shadow-sm transition hover:bg-slate-800 sm:w-auto">
          Open FAQ
        </Link>
      </div>
      <div className="mt-4 grid gap-3 xl:grid-cols-3">
        <div className="rounded-2xl bg-slate-50 p-4 ring-1 ring-slate-200">
          <div className="flex items-center gap-2 text-xs font-black uppercase tracking-[0.14em] text-slate-500"><BookOpenCheck className="size-3 text-amber-700" /> Answers</div>
          <p className="mt-2 text-sm font-bold leading-6 text-slate-700">Create FAQ entries with a question, category, and clear answer.</p>
        </div>
        <div className="rounded-2xl bg-slate-50 p-4 ring-1 ring-slate-200">
          ...
          <p className="mt-2 text-sm font-bold leading-6 text-slate-700">Members can search and filter FAQs by category.</p>
        </div>
        <div className="rounded-2xl bg-slate-50 p-4 ring-1 ring-slate-200">
          ...
          <p className="mt-2 text-sm font-bold leading-6 text-slate-700">Pin critical rules, instructions, and answers for quick access.</p>
        </div>
      </div>
    </div>,
    sectionHost,
