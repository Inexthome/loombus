...
function RequestStatusControls({ request, statusSavingId, onStatusChange }: { request: RequestItem; statusSavingId: string; onStatusChange: (request: RequestItem, status: RequestStatus) => void }) {
  return (
    <div className="rounded-2xl border border-slate-200 bg-slate-50 p-3">
      ...
      <div className="mt-3 flex flex-wrap gap-2">
        {REQUEST_STATUSES.map((entry) => (
          <button key={entry.value} type="button" onClick={() => onStatusChange(request, entry.value)} disabled={request.status === entry.value || Boolean(statusSavingId)} className="inline-flex items-center gap-1 rounded-xl border border-slate-200 bg-white px-3 py-2 text-xs font-black text-slate-700 transition hover:bg-slate-100 disabled:cursor-not-allowed disabled:opacity-45">
            <CheckCircle2 className="size-3" /> {statusSavingId === `${request.id}:${entry.value}` ? "Updating..." : entry.label}
          </button>
        ))}
      </div>
    </div>
...
function RequestDetailPanel({ request, canManage, statusSavingId, onClose, onStatusChange }: { request: RequestItem; canManage: boolean; statusSavingId: string; onClose: () => void; onStatusChange: (request: RequestItem, status: RequestStatus) => void }) {
  const status = getStatus(request.status);

  return (
    <section className="rounded-[1.5rem] border border-amber-200 bg-white p-5 shadow-sm ring-1 ring-amber-100">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div className="min-w-0">
          <p className="text-xs font-black uppercase tracking-[0.18em] text-amber-700">Request detail</p>
          <h2 className="mt-2 text-2xl font-black text-slate-950">{request.title}</h2>
        </div>
        <button type="button" onClick={onClose} className="rounded-full bg-slate-100 px-4 py-2 text-xs font-black text-slate-700 ring-1 ring-slate-200 transition hover:text-amber-700">
          Close
        </button>
      </div>
      ...
      <div className="mt-4 flex flex-wrap gap-2">
        ...
        <span className="rounded-full bg-slate-100 px-3 py-1 text-[10px] font-black uppercase tracking-[0.12em] text-slate-500 ring-1 ring-slate-200">{formatLabel(request.category)}</span>
        <span className="rounded-full bg-slate-100 px-3 py-1 text-[10px] font-black uppercase tracking-[0.12em] text-slate-500 ring-1 ring-slate-200">{formatLabel(request.priority)}</span>
        <span className="rounded-full bg-slate-100 px-3 py-1 text-[10px] font-black uppercase tracking-[0.12em] text-slate-500 ring-1 ring-slate-200">Submitted {request.createdAt}</span>
      </div>

      <div className="mt-5 rounded-2xl border border-slate-200 bg-slate-50 p-4">
        <p className="text-[10px] font-black uppercase tracking-[0.16em] text-slate-500">Full details</p>
        <p className="mt-3 whitespace-pre-wrap text-sm font-semibold leading-7 text-slate-700">{request.details}</p>
      </div>

      {canManage ? (
        <div className="mt-5">
          <RequestStatusControls request={request} statusSavingId={statusSavingId} onStatusChange={onStatusChange} />
        </div>
      ) : (
        <p className="mt-5 rounded-2xl bg-slate-50 px-4 py-3 text-sm font-bold text-slate-600 ring-1 ring-slate-200">Members can view request details and status. Only room owners/admins can update status.</p>
      )}
    </section>
  );
}

function RequestList({ items, selectedRequestId, onSelect }: { items: RequestItem[]; selectedRequestId: string; onSelect: (request: RequestItem) => void }) {
  if (items.length === 0) {
    return (
  ...
  return (
    <div className="grid gap-3">
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          <p className="text-xs font-black uppercase tracking-[0.18em] text-amber-700">Visible requests</p>
      ...
      {items.map((item) => {
        ...
        return (
          <article key={item.id} className={`rounded-[1.25rem] border bg-white p-4 shadow-sm ${isSelected ? "border-amber-300 ring-2 ring-amber-100" : "border-slate-200"}`}>
            <div className="flex flex-wrap items-start justify-between gap-3">
              <div className="min-w-0">
                ...
                <p className="mt-2 line-clamp-3 text-sm font-semibold leading-6 text-slate-600">{item.details}</p>
                <p className="mt-3 text-xs font-bold text-slate-400">Submitted {item.createdAt}</p>
              </div>
              <button type="button" onClick={() => onSelect(item)} className="rounded-full bg-amber-50 px-4 py-2 text-xs font-black text-amber-700 ring-1 ring-amber-100 transition hover:bg-amber-100">
...
export function RoomRequestsPage() {
  ...
  const selectedRequest = useMemo(() => items.find((item) => item.id === selectedRequestId) ?? null, [items, selectedRequestId]);
        ...
        <section className="mt-6 overflow-hidden rounded-[2rem] border border-slate-200 bg-white shadow-sm">
          ...
          {state === "ready" && (
            <div className="grid gap-5 p-5 sm:p-6">
              ...
              {selectedRequest && <RequestDetailPanel request={selectedRequest} canManage={canManageRoom} statusSavingId={statusSavingId} onClose={() => setSelectedRequestId("")} onStatusChange={handleStatusChange} />}
              ...
              <RequestList items={filteredItems} selectedRequestId={selectedRequestId} onSelect={(request) => setSelectedRequestId(request.id)} />
            </div>
          )}
