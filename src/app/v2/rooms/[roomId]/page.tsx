...
const PLACEHOLDER_EVENTS = [
  { title: "Upcoming room event", meta: "Calendar wiring next", body: "Owners will be able to add meetings, resident events, classes, support sessions, or maintenance windows here." },
  { title: "Room reminders", meta: "Planned", body: "Members should eventually receive event reminders and updates inside Loombus notifications." },
];
...
function getPlanLimit(planKey: string, fallback: string) {
  ...
  const limits: Record<string, string> = {
    ...
    starter: "50 members",
    pro: "250 members",
    organization: "Up to 3 rooms, 500 members",
    organization_plus: "Up to 10 rooms, 2,000 members",
    organization_enterprise: "Unlimited/custom rooms, large membership",
  };
...
export default function V2RoomDetailPage() {
  ...
  async function loadRoom() {
    ...
    try {
      ...
      if (nextRoom.isPrivate && !nextIsJoined) {
        ...
        setMessage(
          inviteCode
            ? "This private room invite is ready to accept. Private content stays hidden until membership is created."
            : "This private room is invite-only. You can request access from the room owner.",
        );
        return;
      }
  ...
  async function handleUpdateRoom(event: React.FormEvent<HTMLFormElement>) {
    ...
    setMessage("");
    try {
      const { error } = await supabase.from("rooms").update({ name: editName.trim(), description: editDescription.trim() }).eq("id", room.id);
      if (error) throw error;
  ...
  const inviteAcceptCard = inviteCode && !isJoined && !isOwner ? (
    <section className="rounded-[1.5rem] border border-amber-200 bg-amber-50 p-5 shadow-sm">
      <h2 className="flex items-center gap-2 text-sm font-black uppercase tracking-[0.18em] text-amber-800">
          ...
          <section className="mt-6 overflow-hidden rounded-[2rem] border border-slate-200 bg-white shadow-sm">
            <div className={`${styles.detailHero} bg-gradient-to-br from-slate-950 via-slate-900 to-amber-700 p-6 text-white sm:p-8`}>
              <div className="flex flex-wrap items-start justify-between gap-4">
                <div>
                  <p className={`${styles.detailEyebrow} text-xs font-black uppercase tracking-[0.24em] text-amber-200`}>Private Room Hub</p>
              ...
              <div className="mt-6 flex flex-wrap gap-3 text-xs font-black uppercase tracking-[0.12em]">
                ...
                <span className={`${styles.detailPill} rounded-full bg-white/10 px-3 py-1 text-amber-50 ring-1 ring-white/15`}>{getPlanName(room.subscriptionPlan)}</span>
                <span className={`${styles.detailPill} rounded-full bg-white/10 px-3 py-1 text-amber-50 ring-1 ring-white/15`}>{room.subscriptionStatus}</span>
              ...
              <div className="space-y-6">
                <section id="overview" className="rounded-[1.5rem] border border-slate-200 bg-slate-50 p-5">
                  <div className="flex flex-wrap items-start justify-between gap-3">
                    <div>
                      ...
                      <p className="mt-2 text-sm leading-6 text-slate-600">A private operating space for discussions, updates, events, resources, members, services, and billing.</p>
                    </div>
                    <span className="rounded-full bg-white px-3 py-1 text-xs font-black uppercase tracking-[0.12em] text-slate-600 ring-1 ring-slate-200">{isOwner ? "Owner view" : "Member view"}</span>
                  </div>
                  <div className="mt-5 grid gap-3 md:grid-cols-4">
                    <div className="rounded-2xl bg-white p-4 ring-1 ring-slate-200"><p className="text-xs font-black uppercase tracking-[0.14em] text-slate-400">Members</p><p className="mt-2 text-2xl font-black text-slate-950">{Math.max(room.memberCount, members.length)}</p></div>
                    <div className="rounded-2xl bg-white p-4 ring-1 ring-slate-200"><p className="text-xs font-black uppercase tracking-[0.14em] text-slate-400">Posts</p><p className="mt-2 text-2xl font-black text-slate-950">{posts.length}</p></div>
                    <div className="rounded-2xl bg-white p-4 ring-1 ring-slate-200"><p className="text-xs font-black uppercase tracking-[0.14em] text-slate-400">Requests</p><p className="mt-2 text-2xl font-black text-slate-950">{pendingApplications.length}</p></div>
                    <div className="rounded-2xl bg-white p-4 ring-1 ring-slate-200"><p className="text-xs font-black uppercase tracking-[0.14em] text-slate-400">Updated</p><p className="mt-2 text-base font-black text-slate-950">{formatRelativeTime(room.updatedAt)}</p></div>
                  </div>
                </section>
                ...
                <section className="grid gap-3 md:grid-cols-2 xl:grid-cols-5">
                  {ROOM_HUB_AREAS.map((area) => {
                    ...
                    return (
                      <a key={area.id} href={`#${area.id}`} className="rounded-[1.25rem] border border-slate-200 bg-white p-4 shadow-sm transition hover:-translate-y-0.5 hover:border-amber-200 hover:shadow-md">
                        <Icon className="size-5 text-amber-700" />
                        <h3 className="mt-3 text-sm font-black text-slate-950">{area.label}</h3>
                ...
                <section id="discussions" className="rounded-[1.5rem] border border-slate-200 bg-white p-5 shadow-sm">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="mt-1 text-sm text-slate-600">These room posts do not appear on the public Loombus discussion page.</p>
                    </div>
                  </div>
                  ...
                  {canPost ? (
                    ...
                    <form onSubmit={handleCreateApplication} className="mt-5 rounded-[1.25rem] border border-slate-200 bg-slate-50 p-5">
                      <h3 className="text-sm font-black uppercase tracking-[0.18em] text-slate-500">Request room access</h3>
                      ...
                      {currentUserApplication ? (
                        ...
                        <>
                          ...
                          <button type="submit" disabled={saving} className="mt-3 inline-flex items-center gap-2 rounded-2xl bg-slate-950 px-5 py-3 text-sm font-black text-white shadow-sm transition hover:bg-slate-800 disabled:cursor-not-allowed disabled:opacity-50">
                            <UserPlus className="size-4" />
                            Send access request
                          </button>
                        </>
                    ...
                    <section className="mt-5 rounded-[1.25rem] border border-slate-200 bg-slate-50 p-5">
                      <h3 className="text-sm font-black uppercase tracking-[0.18em] text-slate-500">Room access required</h3>
                  ...
                  <div className="mt-5 space-y-4">
                    ...
                    {posts.length === 0 && (
                      <div className="rounded-[1.5rem] border border-dashed border-slate-300 bg-white p-6 text-center">
                        ...
                        <h3 className="mt-3 text-lg font-black text-slate-950">No room activity yet</h3>
                ...
                <section id="calendar" className="rounded-[1.5rem] border border-slate-200 bg-white p-5 shadow-sm">
                  <div className="flex items-center gap-3"><CalendarDays className="size-5 text-amber-700" /><div><h2 className="text-sm font-black uppercase tracking-[0.18em] text-slate-500">Calendar</h2><p className="mt-1 text-sm text-slate-600">Events and room dates are staged here before the event table is wired.</p></div></div>
                  <div className="mt-4 grid gap-3 md:grid-cols-2">
                    {PLACEHOLDER_EVENTS.map((event) => <div key={event.title} className="rounded-2xl bg-slate-50 p-4 ring-1 ring-slate-200"><p className="text-xs font-black uppercase tracking-[0.14em] text-amber-700">{event.meta}</p><h3 className="mt-2 text-sm font-black text-slate-950">{event.title}</h3><p className="mt-2 text-sm leading-6 text-slate-600">{event.body}</p></div>)}
                  </div>
                  {isOwner && <button type="button" disabled className="mt-4 inline-flex items-center gap-2 rounded-2xl bg-slate-100 px-4 py-3 text-sm font-black text-slate-500"><CalendarDays className="size-4" />Add event coming next</button>}
                </section>

                <section id="announcements" className="rounded-[1.5rem] border border-slate-200 bg-white p-5 shadow-sm">
                  <div className="flex items-center gap-3"><Megaphone className="size-5 text-amber-700" /><div><h2 className="text-sm font-black uppercase tracking-[0.18em] text-slate-500">Announcements</h2><p className="mt-1 text-sm text-slate-600">Pinned notices give rooms a place for updates that should not get buried.</p></div></div>
                  <div className="mt-4 grid gap-3 md:grid-cols-2">
                    {PLACEHOLDER_ANNOUNCEMENTS.map((announcement) => <article key={announcement.title} className="rounded-2xl border border-amber-100 bg-amber-50 p-4"><h3 className="text-sm font-black text-amber-950">{announcement.title}</h3><p className="mt-2 text-sm leading-6 text-amber-900">{announcement.body}</p></article>)}
                  </div>
                </section>

                <section id="requests" className="rounded-[1.5rem] border border-slate-200 bg-white p-5 shadow-sm">
                  <div className="flex items-center gap-3"><ClipboardList className="size-5 text-amber-700" /><div><h2 className="text-sm font-black uppercase tracking-[0.18em] text-slate-500">Requests</h2><p className="mt-1 text-sm text-slate-600">Access requests are live now. Maintenance/help requests can use this same structured area later.</p></div></div>
                  <div className="mt-4 grid gap-3 md:grid-cols-2">
                    {PLACEHOLDER_REQUESTS.map((request) => <div key={request.title} className="rounded-2xl bg-slate-50 p-4 ring-1 ring-slate-200"><span className="rounded-full bg-white px-2 py-1 text-[10px] font-black uppercase tracking-[0.12em] text-slate-500 ring-1 ring-slate-200">{request.status}</span><h3 className="mt-3 text-sm font-black text-slate-950">{request.title}</h3><p className="mt-2 text-sm leading-6 text-slate-600">{request.body}</p></div>)}
                  </div>
                </section>

                <section id="resources" className="rounded-[1.5rem] border border-slate-200 bg-white p-5 shadow-sm">
                  <div className="flex items-center gap-3"><FileText className="size-5 text-amber-700" /><div><h2 className="text-sm font-black uppercase tracking-[0.18em] text-slate-500">Resources</h2><p className="mt-1 text-sm text-slate-600">Documents, rules, links, forms, and help articles for this room.</p></div></div>
                  <div className="mt-4 grid gap-3 md:grid-cols-3">
                    {PLACEHOLDER_RESOURCES.map((resource) => <div key={resource.title} className="rounded-2xl bg-slate-50 p-4 ring-1 ring-slate-200"><h3 className="text-sm font-black text-slate-950">{resource.title}</h3><p className="mt-2 text-sm leading-6 text-slate-600">{resource.body}</p></div>)}
                  </div>
                </section>

                <section id="services" className="rounded-[1.5rem] border border-slate-200 bg-white p-5 shadow-sm">
                  <div className="flex items-center gap-3"><ShoppingBag className="size-5 text-amber-700" /><div><h2 className="text-sm font-black uppercase tracking-[0.18em] text-slate-500">Services / Store</h2><p className="mt-1 text-sm text-slate-600">A future space for room-specific services, offers, booking, and storefronts.</p></div></div>
                  <div className="mt-4 grid gap-3 md:grid-cols-2">
                    {PLACEHOLDER_SERVICES.map((service) => <div key={service.title} className="rounded-2xl bg-slate-50 p-4 ring-1 ring-slate-200"><h3 className="text-sm font-black text-slate-950">{service.title}</h3><p className="mt-2 text-sm leading-6 text-slate-600">{service.body}</p></div>)}
                  </div>
                </section>

                <section id="settings" className="rounded-[1.5rem] border border-slate-200 bg-white p-5 shadow-sm">
                  <div className="flex items-center gap-3"><Settings className="size-5 text-amber-700" /><div><h2 className="text-sm font-black uppercase tracking-[0.18em] text-slate-500">Settings</h2><p className="mt-1 text-sm text-slate-600">Room identity, privacy, permissions, and invite tools.</p></div></div>
                  {isOwner ? (
                    <form onSubmit={handleUpdateRoom} className="mt-5 rounded-[1.25rem] border border-slate-200 bg-slate-50 p-4">
                      <div className="flex flex-wrap items-start justify-between gap-3">
                        <div><h3 className="text-sm font-black text-slate-950">Owner settings</h3><p className="mt-1 text-sm leading-6 text-slate-600">Edit the room name and description shown to members.</p></div>
                        <Link href={`/rooms/${room.id}/invite`} className="inline-flex items-center gap-2 rounded-2xl border border-slate-200 bg-white px-4 py-2 text-xs font-black text-slate-700 transition hover:bg-slate-50"><Link2 className="size-4" />Invite link</Link>
                      </div>
                  ...
                  ) : <p className="mt-4 rounded-2xl bg-slate-50 p-4 text-sm font-semibold text-slate-600 ring-1 ring-slate-200">Only room owners can edit settings.</p>}
                </section>
              </div>

              <aside className="space-y-4">
                <section id="members" className="rounded-[1.5rem] border border-slate-200 bg-white p-5 shadow-sm">
                  <div className="flex items-center justify-between gap-3"><h2 className="text-sm font-black uppercase tracking-[0.16em] text-slate-500">Membership</h2><Users className="size-4 text-amber-700" /></div>
                  <p className="mt-3 text-sm leading-6 text-slate-600">{isOwner ? "You own this room." : room.isPrivate ? "Private room membership is controlled by owner approval." : "Join this room to keep it in Your Rooms."}</p>
                  <div className="mt-4">
                    {isOwner ? <span className="inline-flex rounded-2xl bg-emerald-50 px-4 py-2 text-sm font-black text-emerald-700">Owner</span> : room.isPrivate ? <span className="inline-flex rounded-2xl bg-slate-100 px-4 py-2 text-sm font-black text-slate-600">Invite only</span> : isJoined ? <button type="button" onClick={handleLeaveRoom} disabled={saving} className="w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm font-black text-slate-700 transition hover:bg-slate-50 disabled:opacity-50">Leave room</button> : <button type="button" onClick={handleJoinRoom} disabled={saving} className="w-full rounded-2xl bg-slate-950 px-4 py-3 text-sm font-black text-white shadow-sm transition hover:bg-slate-800 disabled:opacity-50">Join room</button>}
                  </div>
                </section>
                      ...
                      {pendingApplications.map((application) => (
                        <div key={application.id} className="rounded-2xl bg-white p-3 text-xs font-semibold text-slate-600 ring-1 ring-amber-100">
                          ...
                          <div className="mt-2 flex gap-2">
                            <button type="button" onClick={() => handleReviewApplication(application, "approved")} disabled={saving} className="inline-flex flex-1 items-center justify-center gap-1 rounded-xl bg-emerald-600 px-3 py-2 text-xs font-black text-white disabled:opacity-50"><CheckCircle2 className="size-3" />Approve</button>
                            <button type="button" onClick={() => handleReviewApplication(application, "declined")} disabled={saving} className="inline-flex flex-1 items-center justify-center gap-1 rounded-xl bg-white px-3 py-2 text-xs font-black text-red-700 ring-1 ring-red-100 disabled:opacity-50"><X className="size-3" />Decline</button>
                          </div>
                        </div>
                      ))}
                    </div>
                ...
                <section id="billing" className="rounded-[1.5rem] border border-slate-200 bg-white p-5 shadow-sm">
                  <div className="flex items-center justify-between gap-3"><h2 className="text-sm font-black uppercase tracking-[0.16em] text-slate-500">Billing</h2><CreditCard className="size-4 text-amber-700" /></div>
                  <dl className="mt-4 space-y-3 text-sm">
                    <div className="flex justify-between gap-3"><dt className="text-slate-500">Plan</dt><dd className="font-black text-slate-900">{getPlanName(room.subscriptionPlan)}</dd></div>
                    <div className="flex justify-between gap-3"><dt className="text-slate-500">Status</dt><dd className="font-black text-slate-900">{room.subscriptionStatus}</dd></div>
                    <div className="gap-3"><dt className="text-slate-500">Limit</dt><dd className="mt-1 font-black text-slate-900">{getPlanLimit(room.subscriptionPlan, room.memberLimitLabel)}</dd></div>
                  </dl>
                  {room.subscriptionStatus !== "active" && <p className="mt-4 rounded-2xl bg-amber-50 p-3 text-xs font-bold leading-5 text-amber-900 ring-1 ring-amber-100">Checkout is not completed yet. The room exists, but billing lock rules will be added in a later PR.</p>}
                </section>
