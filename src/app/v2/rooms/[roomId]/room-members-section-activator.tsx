...
function applyLiveCopy(section: HTMLElement) {
  ...
  replaceTextIncludes(section, "Planned", "LIVE");
  replaceTextIncludes(section, "Member directory, access roles, and private invite controls.", "Member directory, access roles, and private room controls are live now.");
}

function resolveMembersSection() {
  const directSection = document.getElementById("members");
  ...
  const heading = headings.find((element) => {
    ...
    return text === "members" || text === "members / roles" || text === "members/roles" || text === "roles";
  });
  return heading?.closest("section") as HTMLElement | null;
}

export function RoomMembersSectionActivator({ roomId }: { roomId: string }) {
  ...
  useEffect(() => {
    ...
    function activate() {
      const section = resolveMembersSection();
      if (!section) return;

      section.setAttribute("data-room-members-live", "true");
      applyLiveCopy(section);

      const existingHost = section.querySelector<HTMLElement>('[data-room-members-section-host="true"]');
      if (existingHost) {
      ...
      host.setAttribute("data-room-members-section-host", "true");
      host.className = "mt-5";
    ...
    <div className="rounded-[1.25rem] border border-amber-100 bg-amber-50/60 p-4 ring-1 ring-amber-100">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div className="flex items-start gap-3">
          ...
          <div>
            <p className="text-xs font-black uppercase tracking-[0.16em] text-amber-700">Members / Roles is live</p>
            <p className="mt-1 text-sm font-semibold leading-6 text-slate-700">
              Use Members / Roles to manage approved room access, assign operating roles, and see the room permission model.
            </p>
          </div>
        </div>
        <Link href={`/rooms/${roomId}/members`} className="inline-flex items-center justify-center rounded-2xl bg-slate-950 px-4 py-3 text-sm font-black text-white shadow-sm transition hover:bg-slate-800">
          Open Members / Roles
        </Link>
      </div>
    </div>
