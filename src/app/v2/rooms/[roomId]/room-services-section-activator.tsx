...
function applyLiveCopy(section: HTMLElement) {
  ...
  replaceTextIncludes(section, "Planned", "LIVE");
  replaceTextIncludes(section, "Services, bookings, offers, and room storefronts.", "Services, bookings, offers, requests, and room storefronts are live now.");
}

function resolveServicesSection() {
  const directSection = document.getElementById("services");
  ...
  const heading = headings.find((element) => {
    ...
    return text === "services" || text === "services / store" || text === "services/store" || text === "store";
  });
  return heading?.closest("section") as HTMLElement | null;
}

export function RoomServicesSectionActivator({ roomId }: { roomId: string }) {
  ...
  useEffect(() => {
    ...
    function activate() {
      ...
      section.setAttribute("data-room-services-live", "true");
      applyLiveCopy(section);

      const existingHost = section.querySelector<HTMLElement>('[data-room-services-section-host="true"]');
      if (existingHost) {
      ...
      host.setAttribute("data-room-services-section-host", "true");
      host.className = "mt-5";
    ...
    <div className="rounded-[1.25rem] border border-amber-100 bg-amber-50/60 p-4 ring-1 ring-amber-100">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div className="flex items-start gap-3">
          ...
          <div>
            <p className="text-xs font-black uppercase tracking-[0.16em] text-amber-700">Services / Store is live</p>
            <p className="mt-1 text-sm font-semibold leading-6 text-slate-700">
              Use the Services / Store center for private room listings, offers, appointment intents, member service requests, and request tracking.
            </p>
          </div>
        </div>
        <Link href={`/rooms/${roomId}/services`} className="inline-flex items-center justify-center rounded-2xl bg-slate-950 px-4 py-3 text-sm font-black text-white shadow-sm transition hover:bg-slate-800">
          Open Services / Store
        </Link>
      </div>
    </div>
