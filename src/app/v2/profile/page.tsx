...
function FooterCards() {
  return <section className="mt-4 grid gap-3 sm:mt-6 sm:gap-4 md:grid-cols-4">{[{ title: "Identity & Presence", description: "Showcase who you are and how others understand your perspective.", icon: UserRound }, { title: "Contribution Overview", description: "Live counts for discussions, replies, saved items, and signals.", icon: Sparkles }, { title: "Public Visibility", description: "Your profile card reflects visible activity across Loombus.", icon: ShieldCheck }, { title: "Quality Reputation", description: "Build trust through constructive public participation.", icon: Award }].map((item) => { const Icon = item.icon; return <div key={item.title} className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:rounded-3xl sm:p-5"><div className="grid size-9 place-items-center rounded-xl bg-amber-50 text-amber-800 ring-1 ring-amber-200 sm:size-11 sm:rounded-2xl"><Icon className="size-5 sm:size-6" /></div><h3 className="mt-2 font-black text-slate-950 sm:mt-3">{item.title}</h3><p className="mt-1 text-sm leading-5 text-slate-500 sm:leading-6">{item.description}</p></div>; })}</section>;
}
