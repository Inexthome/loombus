...
import { Bell, Home, Loader2, Lock, MessageCircle, Plus, Search, Settings, Users } from "lucide-react";
import { supabase } from "@/lib/supabase/client";
import { V2AppearanceControl } from "../v2-appearance-control";

type ShellPayload = {
  version: "v1" | "v2";
  configured: boolean;
  authenticated: boolean;
  flags: { v2_shell: boolean; v2_signal_brief: boolean; v2_rooms: boolean };
};
...
function getDefaultShellPayload(): ShellPayload {
  return {
    ...
    configured: false,
    authenticated: false,
    flags: { v2_shell: false, v2_signal_brief: false, v2_rooms: false },
  };
}
    ...
    <main className="fixed inset-0 z-[80] min-h-screen overflow-y-auto bg-[#f7fbff] text-slate-950">
      ...
      <section className="mx-auto max-w-7xl px-4 pb-28 pt-6 sm:px-6 lg:px-8">
        <header className="mb-6">
          ...
          <p className="mt-2 max-w-2xl text-sm leading-6 text-slate-600">Manage V2 appearance, account preferences, notifications, privacy, and app behavior.</p>
        </header>

        <V2AppearanceControl />

        <section className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
          {["Account Preferences", "Notifications", "Privacy", "Security", "Billing", "Data Controls"].map((item) => (
            <article key={item} className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
              <Settings className="size-6 text-blue-600" />
              <h2 className="mt-3 font-black text-slate-950">{item}</h2>
              <p className="mt-1 text-sm leading-6 text-slate-500">V2 shell space for {item.toLowerCase()}.</p>
            </article>
          ))}
        </section>
      </section>
