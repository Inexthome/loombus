...
const LOOMBUS_GOLD_HOVER = "#c7993f";
const DRAFT_STORAGE_MESSAGE = "Draft storage is not available yet. You can keep editing, but autosave may not run.";
...
function MetadataPicker({
  ...
  const topicOptions = getSelectableTopics();
  const options = activePanel === "topics" ? topicOptions : activePanel === "reality_lens" ? REALITY_LENSES : PURPOSE_LANES;
  ...
  return (
    <div className="absolute left-0 right-0 top-[calc(100%+0.5rem)] z-50 rounded-3xl border border-slate-200 bg-white p-3 shadow-2xl shadow-slate-950/15 ring-1 ring-slate-900/5">
      ...
      {activePanel === "topics" && (
        <button
          type="button"
          onClick={() => setActivePanel("reality_lens")}
...
export default function V2CreateClientPage() {
  ...
  function addSupportingContext(event: ChangeEvent<HTMLInputElement>, kind: SupportingContextKind) {
    ...
    setSupportingContext((current) => {
      const nextItems = files.map((file) => ({ id: `${kind}-${file.name}-${file.size}-${file.lastModified}`, name: file.name, size: file.size, type: file.type || "Unknown type", kind }));
      return [...current, ...nextItems].filter((item, index, list) => list.findIndex((candidate) => candidate.id === item.id) === index).slice(0, 6);
    });
    setDraftMessage("");
    event.target.value = "";
  }
  ...
  async function loadSavedDraft(nextUserId: string) {
    ...
    try {
      ...
      if (error) {
        ...
        setDraftMessage(DRAFT_STORAGE_MESSAGE);
        return;
      }
      ...
      setTopic(isOtherOption(draft.topic ?? "") ? "" : (draft.topic ?? ""));
      ...
      setDraftMessage("Unable to load this draft safely.");
    } finally {
  ...
  async function persistDraft({ manual = false }: { manual?: boolean } = {}) {
    if (!userId) {
      if (manual) setDraftMessage("Sign in is required before saving a draft.");
      return false;
    }
    setDraftLoading(true);
    if (manual) setDraftMessage("");
    else setAutosaveStatus("Autosaving...");

    try {
      const { data, error } = await supabase
        .from("loombus_v2_create_drafts")
        .upsert(
          { user_id: userId, title, topic, body: buildDraftBody({ body, purpose, realityLens, purposeLane }), tags, mode },
          { onConflict: "user_id" }
        )
      ...
      if (manual) setDraftMessage("Draft could not be saved.");
      else setAutosaveStatus("Autosave failed");
      return false;
    } finally {
      setDraftLoading(false);
    }
  ...
  async function clearDraft() {
    if (!userId) {
      setDraftMessage("Sign in is required before clearing a draft.");
      return;
    }
    setDraftLoading(true);
    setDraftMessage("");
    ...
    try {
      ...
      setDraftMessage("Unable to clear this draft safely.");
    } finally {
      setDraftLoading(false);
    }
  }
  ...
  async function loadShell() {
    ...
    try {
      ...
      setAutosaveStatus("Autosave unavailable");
      setMessage("Unable to verify Create access.");
    } finally {
      setLoading(false);
    }
  }
  ...
  useEffect(() => {
    const canAutosave = userId && !loading && draftHydrated && payload?.configured && payload.flags.v2_shell && payload.version === "v2" && hasDraftContent;
    if (!canAutosave) return;
    setAutosaveStatus("Autosave pending...");
    const timer = window.setTimeout(() => persistDraft({ manual: false }), AUTOSAVE_DELAY_MS);
    return () => window.clearTimeout(timer);
  }, [draftFingerprint, draftHydrated, hasDraftContent, loading, payload, userId]);

  if (loading) return <V2ShellGateCard title="Checking Create" message="Loombus is loading Create." loading />;
  if (message) return <V2ShellGateCard title="Create check failed safely" message={message} payload={payload} />;
  if (!payload?.authenticated) return <V2ShellGateCard title="Sign in required" message="Sign in to create a Loombus discussion." payload={payload} />;
  if (!payload.configured || !payload.flags.v2_shell || payload.version !== "v2") return <V2ShellGateCard title="Create is unavailable" message="This account cannot access Create yet." payload={payload} />;
    ...
    <main className="fixed inset-0 z-[80] min-h-screen overflow-y-auto bg-[#f7fbff] loombus-v2-page-bg text-slate-950">
      ...
      <section className="mx-auto max-w-7xl px-4 pb-28 pt-6 sm:px-6 lg:px-8">
        <header className="mb-5">
          ...
          <p className="mt-2 max-w-2xl text-sm leading-6 text-slate-600">Start a meaningful conversation. Give your community something worth discussing.</p>
        </header>

        <section className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_320px]">
          <div className="space-y-5">
            <section className="rounded-[2rem] border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
              {draftMessage && <div className="mb-5 rounded-2xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm font-medium text-amber-800">{draftMessage}</div>}
              ...
              <div className="grid gap-4">
                ...
                <div className="relative" ref={pickerRef}>
                  ...
                  <button type="button" onClick={() => { setActivePickerPanel("topics"); setPickerOpen((current) => !current); }} className="flex w-full items-center gap-3 rounded-xl border border-slate-200 bg-white px-4 py-3 text-left text-sm text-slate-900 shadow-sm transition hover:border-amber-300 hover:bg-amber-50 focus:border-amber-400 focus:outline-none focus:ring-4 focus:ring-amber-100">
                    <Plus className="size-5 text-amber-800" />
                    <span className={topic ? "font-bold text-slate-900" : "text-slate-400"}>{topic || "Choose an approved topic"}</span>
                  </button>
                  <MetadataPicker open={pickerOpen} activePanel={activePickerPanel} setActivePanel={setActivePickerPanel} onClose={() => setPickerOpen(false)} onSelectTopic={setTopic} onSelectRealityLens={setRealityLens} onSelectPurposeLane={setPurposeLane} />
                  ...
                  <p className="mt-2 text-xs font-medium text-slate-500">Use the plus menu to choose a topic. Use Other only to choose Reality Lens and Purpose Lane.</p>
                </div>
                ...
                <div>
                  ...
                  <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
                    {DISCUSSION_MODES.map((option) => {
                      ...
                      return (
                        <button key={option.key} type="button" onClick={() => setMode(option.key)} className={`rounded-2xl border p-4 text-center transition ${selected ? "border-amber-400 bg-amber-50 shadow-sm ring-2 ring-amber-100" : "border-slate-200 bg-white hover:border-amber-200 hover:bg-amber-50/50"}`}>
                ...
                <label className="block">
                  ...
                  <input value={purpose} onChange={(event) => setPurpose(event.target.value)} placeholder="What do you hope to achieve with this discussion?" className="w-full rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm text-slate-900 outline-none transition placeholder:text-slate-400 focus:border-amber-400 focus:ring-4 focus:ring-amber-100" />
                  <p className="mt-2 text-xs font-medium text-slate-500">Define your intent to guide better responses.</p>
                </label>

                <label className="block">
                  <div className="mb-2 flex items-center justify-between gap-3"><span className="text-sm font-bold text-slate-700">Body <span className="text-red-500">*</span></span><span className="text-xs font-semibold text-slate-400">{body.length}/3000</span></div>
                ...
                <section className="rounded-2xl border border-amber-200 bg-amber-50/50 p-4">
                  <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                    <div><h2 className="flex items-center gap-2 text-sm font-black text-amber-950"><Brain className="size-4" />AI draft tools</h2><p className="mt-1 text-xs font-semibold leading-5 text-amber-900">Check discussion quality or rewrite the body for clarity before review.</p></div>
                    <div className="flex flex-wrap gap-2">
                      <button type="button" onClick={runQualityCheck} className="inline-flex items-center gap-2 rounded-xl bg-white px-3 py-2 text-xs font-black text-amber-800 shadow-sm ring-1 ring-amber-100 transition hover:bg-amber-100"><Sparkles className="size-4" />Discussion quality check</button>
                      <button type="button" onClick={rewriteForClarity} className="inline-flex items-center gap-2 rounded-xl bg-[#d6a84f] px-3 py-2 text-xs font-black text-slate-950 shadow-sm transition hover:bg-[#c7993f]"><WandSparkles className="size-4" />Rewrite for clarity</button>
                    </div>
                  </div>
                  {aiMessage && <p className="mt-3 text-xs font-bold text-amber-950">{aiMessage}</p>}
                  {aiFindings.length > 0 && <div className="mt-3 grid gap-2 sm:grid-cols-2">{aiFindings.map((finding) => <div key={finding.label} className="rounded-xl bg-white px-3 py-2 text-xs shadow-sm ring-1 ring-amber-100"><p className={`flex items-center gap-2 font-black ${finding.done ? "text-emerald-700" : "text-amber-700"}`}>{finding.done ? <CheckCircle2 className="size-4" /> : <Circle className="size-4" />}{finding.label}</p><p className="mt-1 leading-5 text-slate-600">{finding.detail}</p></div>)}</div>}
                </section>

                <label className="block">
                  <div className="mb-2 flex items-center justify-between gap-3"><span className="text-sm font-bold text-slate-700">Tags</span><span className="text-xs font-semibold text-slate-400">{getTagCount(tags)}/5</span></div>
            ...
            <section className="rounded-[2rem] border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
              ...
              <p className="mt-1 text-xs font-medium text-slate-500">Attach files or video context to support the draft setup. Publish-time persistence will be completed in the next wiring pass.</p>
              ...
              <div className="mt-4 grid gap-3 sm:grid-cols-2">
                <button type="button" onClick={() => fileInputRef.current?.click()} className="rounded-2xl border border-slate-200 bg-slate-50 p-4 text-left transition hover:border-amber-300 hover:bg-amber-50"><div className="flex items-center gap-3"><span className="grid size-10 place-items-center rounded-2xl bg-amber-50 text-amber-700"><Paperclip className="size-5" /></span><div><p className="text-sm font-black text-slate-800">Attach Files</p><p className="mt-1 text-xs leading-5 text-slate-500">Images, PDFs, text, or documents.</p></div></div></button>
                <button type="button" onClick={() => videoInputRef.current?.click()} className="rounded-2xl border border-slate-200 bg-slate-50 p-4 text-left transition hover:border-amber-300 hover:bg-amber-50"><div className="flex items-center gap-3"><span className="grid size-10 place-items-center rounded-2xl bg-amber-50 text-amber-700"><Mic className="size-5" /></span><div><p className="text-sm font-black text-slate-800">Video Context</p><p className="mt-1 text-xs leading-5 text-slate-500">Select a video file to stage with the draft.</p></div></div></button>
              </div>
              {supportingContext.length > 0 && <div className="mt-4 space-y-2">{supportingContext.map((item) => <div key={item.id} className="flex items-center justify-between gap-3 rounded-2xl border border-slate-200 bg-white px-3 py-2 text-sm"><div className="min-w-0"><p className="truncate font-black text-slate-800">{item.name}</p><p className="text-xs font-semibold text-slate-400">{item.kind === "video" ? "Video" : "File"} · {formatFileSize(item.size)}</p></div><button type="button" onClick={() => removeSupportingContext(item.id)} className="grid size-8 shrink-0 place-items-center rounded-full text-slate-400 transition hover:bg-red-50 hover:text-red-600" aria-label={`Remove ${item.name}`}><X className="size-4" /></button></div>)}</div>}
              ...
              <div className="mt-5 flex flex-col gap-3 border-t border-slate-100 pt-5 sm:flex-row sm:items-center sm:justify-between">
                ...
                <Link href="/v2/create/review" className="inline-flex items-center justify-center gap-2 rounded-2xl px-5 py-2.5 text-sm font-black text-slate-950 shadow-sm transition" style={{ backgroundColor: LOOMBUS_GOLD }} onMouseEnter={(event) => { event.currentTarget.style.backgroundColor = LOOMBUS_GOLD_HOVER; }} onMouseLeave={(event) => { event.currentTarget.style.backgroundColor = LOOMBUS_GOLD; }}><Send className="size-4" />Review Draft</Link>
              </div>
            </section>
          </div>
          ...
          <aside className="space-y-4">
            <section className="rounded-[2rem] border border-slate-200 bg-white p-5 shadow-sm">
              ...
              <div className="mt-5 space-y-5">{readiness.checks.map((check, index) => <div key={check.label} className="flex gap-3"><span className={`grid size-8 shrink-0 place-items-center rounded-full text-xs font-black ${check.done ? "text-slate-950" : "bg-amber-50 text-amber-800"}`} style={check.done ? { backgroundColor: LOOMBUS_GOLD } : undefined}>{index + 1}</span><div><p className="text-sm font-black text-slate-800">{check.label}</p><p className="mt-1 text-xs leading-5 text-slate-500">{check.done ? "Ready" : "Needs attention before review"}</p></div></div>)}</div>
              <div className="mt-5 h-2 overflow-hidden rounded-full bg-slate-100"><div className="h-full rounded-full" style={{ width: `${readiness.percent}%`, backgroundColor: LOOMBUS_GOLD }} /></div>
              <p className="mt-3 text-xs font-semibold text-slate-500">{readiness.completed} of {readiness.total} checks complete.</p>
            </section>

            <section className="rounded-[2rem] border border-slate-200 bg-white p-5 shadow-sm">
              <div className="flex items-center gap-3"><Save className="size-5 text-amber-700" /><h2 className="font-black text-slate-950">Private draft</h2></div>
            ...
            <section className="rounded-[2rem] border border-slate-200 bg-white p-5 shadow-sm">
              ...
              <div className="flex flex-col gap-3">
                <button type="button" onClick={copyPreviewDraft} className="rounded-2xl border border-slate-200 px-4 py-2 text-sm font-black text-amber-800 transition hover:border-amber-300 hover:bg-amber-50">Copy draft</button>
                <Link href="/v2/create/review" className="rounded-2xl px-4 py-2 text-center text-sm font-black text-slate-950 transition" style={{ backgroundColor: LOOMBUS_GOLD }}>Review draft</Link>
              </div>
              {copyMessage && <p className="mt-3 text-xs leading-5 text-slate-500">{copyMessage}</p>}
            </section>

            <section className="rounded-[2rem] border border-slate-200 bg-white p-5 shadow-sm">
              <div className="flex items-center gap-3"><FileText className="size-5 text-amber-700" /><h2 className="font-black text-slate-950">Selected framing</h2></div>
              ...
              <div className="mt-4 space-y-2 text-xs font-semibold text-slate-500"><p>Topic: {topic || "Not selected"}</p><p>Reality Lens: {realityLens || "Not selected"}</p><p>Purpose Lane: {purposeLane || "Not selected"}</p><p>Supporting context: {supportingContext.length > 0 ? `${supportingContext.length} staged` : "None staged"}</p></div>
            </section>
          </aside>
        </section>
      </section>
