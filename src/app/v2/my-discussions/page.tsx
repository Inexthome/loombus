...
export default function V2MyDiscussionsPage() {
  ...
  function applyQuickView(filter: MyDiscussionFilter, nextSort?: MyDiscussionSort) { setActiveFilter(filter); if (nextSort) setSortBy(nextSort); setShowFilters(false); setOpenMenuId(null); focusList(); }
  function showAllItems() { setActiveFilter("Published"); setSortBy("recent"); setQuery(""); setShowFilters(false); setOpenMenuId(null); focusList(); }

  useEffect(() => {
    if (!openMenuId) return;
    function handlePointerDown(event: PointerEvent) {
      const target = event.target as HTMLElement | null;
      if (!target?.closest("[data-my-discussion-actions]")) setOpenMenuId(null);
    }
  ...
  async function loadDiscussions(userId: string) {
    ...
    try {
      ...
      const draftRows = drafts.map((draft, index) => ({ id: `draft-${draft.id}`, discussionId: null, draftId: draft.id, title: draft.title?.trim() || "Untitled draft", topic: draft.topic, body: draft.body, created_at: draft.created_at, updated_at: draft.updated_at, edited_at: null, discussion_type: null, status: "Draft", replyCount: 0, viewCount: 0, lastActivityAt: draft.updated_at ?? draft.created_at, accent: getAccent(index + publishedItems.length) } satisfies MyDiscussionItem));
      setItems([...publishedItems, ...draftRows]);
  ...
  async function deleteDraft(draftId: string) { setMessage(""); if (deletingDraftId) return; setDeletingDraftId(draftId); const accessToken = await getAccessToken(); if (!accessToken) { setDeletingDraftId(null); window.location.href = "/v2/login"; return; } const response = await fetch("/api/discussion-drafts", { method: "DELETE", headers: { "Content-Type": "application/json", Authorization: `Bearer ${accessToken}` }, body: JSON.stringify({ draftId }) }); const result = await response.json().catch(() => ({})); setDeletingDraftId(null); if (!response.ok) { setMessage(result.error ?? "Unable to delete draft."); return; } setItems((current) => current.filter((item) => item.draftId !== draftId)); setOpenMenuId(null); setMessage("Draft deleted."); }
  async function deleteDiscussion(discussionId: string) { setMessage(""); if (deletingDiscussionId) return; const confirmed = window.confirm("Delete this discussion? It will leave public view, but moderation and audit records will be preserved."); if (!confirmed) return; setDeletingDiscussionId(discussionId); const accessToken = await getAccessToken(); if (!accessToken) { setDeletingDiscussionId(null); window.location.href = "/v2/login"; return; } const response = await fetch("/api/discussions/delete", { method: "POST", headers: { "Content-Type": "application/json", Authorization: `Bearer ${accessToken}` }, body: JSON.stringify({ discussionId }) }); const result = await response.json().catch(() => ({})); setDeletingDiscussionId(null); if (!response.ok) { setMessage(result.error ?? "Unable to delete discussion."); return; } setItems((current) => current.filter((item) => item.discussionId !== discussionId)); setOpenMenuId(null); setMessage("Discussion deleted."); }
  function resetFilters() { setActiveFilter("Published"); setSortBy("recent"); setQuery(""); setOpenMenuId(null); }
