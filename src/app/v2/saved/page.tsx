...
function SavedItemRow({ item, collections, movingBookmarkId, removingBookmarkId, savingNoteId, noteDraft, onMove, onRemove, onNoteChange, onSaveNote }: {
  item: SavedItem;
    ...
    <article className="flex flex-col gap-4 border-b border-slate-100 py-4 last:border-b-0 sm:flex-row sm:items-start">
      ...
      <div className="min-w-0 flex-1">
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div className="min-w-0"><Link href={`/v2/discussions/${item.id}`} className="text-lg font-black text-slate-950 transition hover:text-blue-700">{item.title}</Link><div className="mt-2 flex flex-wrap gap-2"><span className="rounded-full bg-blue-50 px-3 py-1 text-xs font-bold text-blue-700">{item.topic || "Discussion"}</span><span className={`rounded-full px-3 py-1 text-xs font-bold ${getModeClass(item.discussion_type)}`}>{getModeLabel(item.discussion_type)}</span>{item.collectionId ? <span className="rounded-full bg-slate-50 px-3 py-1 text-xs font-bold text-slate-600">Filed</span> : <span className="rounded-full bg-amber-50 px-3 py-1 text-xs font-bold text-amber-700">Unfiled</span>}</div></div>
...
export default function V2SavedPage() {
  ...
  const collectionCounts = useMemo(() => {
    ...
    for (const collection of collections) counts[collection.id] = 0;
  ...
  }, [activeFilter, collectionNameById, items, noteDrafts, query]);
  ...
  async function loadSaved(userId: string) {
    ...
    try {
      ...
      if (authorIds.length > 0) {
        const { data: profileRows } = await supabase.from("profiles").select("id, full_name, username, avatar_url").in("id", authorIds);
        for (const profile of (profileRows ?? []) as ProfileRow[]) profilesById.set(profile.id, profile);
      }
      if (discussionIds.length > 0) {
        const { data: attachmentRows } = await supabase.from("discussion_attachments").select("discussion_id, public_url, file_name, mime_type, attachment_kind").in("discussion_id", discussionIds).order("sort_order", { ascending: true });
        for (const attachment of (attachmentRows ?? []) as AttachmentRow[]) if (attachment.discussion_id && !attachmentsByDiscussionId.has(attachment.discussion_id)) attachmentsByDiscussionId.set(attachment.discussion_id, attachment);
      }
      const nextItems = bookmarks.map((bookmark) => {
        ...
        return { ...discussion, bookmarkId: bookmark.id, collectionId: bookmark.collection_id ?? null, privateNote: bookmark.private_note ?? null, savedAt: bookmark.created_at, authorName: profile?.full_name?.trim() || profile?.username?.trim() || "Loombus member", authorUsername: profile?.username ? `@${profile.username}` : "Loombus Lab", authorAvatarUrl: profile?.avatar_url ?? null, attachmentUrl: attachment?.public_url ?? null, attachmentName: attachment?.file_name ?? null, attachmentMimeType: attachment?.mime_type ?? null, savedKind: attachment ? "File" : "Discussion" } satisfies SavedItem;
      }).filter((item): item is SavedItem => Boolean(item));
      setItems(nextItems);
      setNoteDrafts(Object.fromEntries(nextItems.map((item) => [item.bookmarkId, item.privateNote ?? ""])));
    } catch {
      setMessage("Unable to load saved items safely. Current Saved remains available."); setItems([]);
  ...
  async function createCollection(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (!accessToken) { setCreatingCollection(false); window.location.href = "/v2/login"; return; }
    const response = await fetch("/api/bookmarks/collections", { method: "POST", headers: { "Content-Type": "application/json", Authorization: `Bearer ${accessToken}` }, body: JSON.stringify({ name }) });
    const result = await response.json().catch(() => ({}));
    setCreatingCollection(false);
    if (!response.ok) { setMessage(result.error ?? "Unable to create folder."); return; }
    setCollections((current) => [...current, result.collection as CollectionRow]);
  ...
  async function moveBookmark(bookmarkId: string, collectionId: string) {
    setMessage(""); setMovingBookmarkId(bookmarkId);
    const accessToken = await getAccessToken();
    if (!accessToken) { setMovingBookmarkId(null); window.location.href = "/v2/login"; return; }
    const nextCollectionId = collectionId === "unfiled" ? null : collectionId;
    const response = await fetch("/api/bookmarks/move", { method: "PATCH", headers: { "Content-Type": "application/json", Authorization: `Bearer ${accessToken}` }, body: JSON.stringify({ bookmarkId, collectionId: nextCollectionId }) });
    const result = await response.json().catch(() => ({}));
    setMovingBookmarkId(null);
    if (!response.ok) { setMessage(result.error ?? "Unable to move saved item."); return; }
    setItems((current) => current.map((item) => item.bookmarkId === bookmarkId ? { ...item, collectionId: nextCollectionId } : item));
    setMessage(nextCollectionId ? "Saved item moved." : "Saved item moved to Unfiled.");
  }
  async function removeBookmark(bookmarkId: string) {
    setMessage(""); setRemovingBookmarkId(bookmarkId);
    const accessToken = await getAccessToken();
    if (!accessToken) { setRemovingBookmarkId(null); window.location.href = "/v2/login"; return; }
    const response = await fetch("/api/bookmarks", { method: "DELETE", headers: { "Content-Type": "application/json", Authorization: `Bearer ${accessToken}` }, body: JSON.stringify({ bookmarkId }) });
    const result = await response.json().catch(() => ({}));
    setRemovingBookmarkId(null);
    if (!response.ok) { setMessage(result.error ?? "Unable to remove saved item."); return; }
    setItems((current) => current.filter((item) => item.bookmarkId !== bookmarkId));
    setMessage("Saved item removed.");
  }
  async function deleteCollection(collectionId: string) {
    setMessage(""); setDeletingCollectionId(collectionId);
    const accessToken = await getAccessToken();
    if (!accessToken) { setDeletingCollectionId(null); window.location.href = "/v2/login"; return; }
    const response = await fetch("/api/bookmarks/collections", { method: "DELETE", headers: { "Content-Type": "application/json", Authorization: `Bearer ${accessToken}` }, body: JSON.stringify({ collectionId }) });
    const result = await response.json().catch(() => ({}));
    setDeletingCollectionId(null);
    if (!response.ok) { setMessage(result.error ?? "Unable to delete folder."); return; }
    setCollections((current) => current.filter((collection) => collection.id !== collectionId));
    setItems((current) => current.map((item) => item.collectionId === collectionId ? { ...item, collectionId: null } : item));
    setMessage("Folder deleted. Its saved items moved to Unfiled.");
  }
  ...
  async function saveNote(bookmarkId: string) {
    ...
    const accessToken = await getAccessToken();
    if (!accessToken) { setSavingNoteId(null); window.location.href = "/v2/login"; return; }
    const response = await fetch("/api/bookmarks/note", { method: "PATCH", headers: { "Content-Type": "application/json", Authorization: `Bearer ${accessToken}` }, body: JSON.stringify({ bookmarkId, note: noteDrafts[bookmarkId] ?? "" }) });
    const result = await response.json().catch(() => ({}));
    setSavingNoteId(null);
    if (!response.ok) { setMessage(result.error ?? "Unable to save note."); return; }
    const nextNote = result.bookmark?.private_note ?? null;
    setItems((current) => current.map((item) => item.bookmarkId === bookmarkId ? { ...item, privateNote: nextNote } : item));
    setNoteDrafts((current) => ({ ...current, [bookmarkId]: nextNote ?? "" }));
    setMessage(nextNote ? "Private note saved." : "Private note cleared.");
  }
  ...
  async function loadShell() {
    ...
    try {
      ...
      if (accessToken && data.session?.user.id && nextPayload.configured && nextPayload.flags.v2_shell && nextPayload.version === "v2") await loadSaved(data.session.user.id);
    } catch { setPayload(getDefaultShellPayload()); setMessage("Unable to verify V2 Saved access. Current Loombus remains on V1."); }
