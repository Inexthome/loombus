...
export default function InsightsClient() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        const discussionIds = [
          ...new Set(replyRows.map((item) => item.discussion_id).filter(Boolean)),
        ];
        ...
        if (!cancelled) {
          setNotice("Some Insights data could not be loaded. Refresh to try again.");
        }
      } finally {
  ...
  const rankedDiscussions = useMemo(
    () =>
      [...impact.discussions].sort(
        (a, b) =>
          b.signalActions - a.signalActions ||
          b.signalContributors - a.signalContributors ||
          b.engagedMembers - a.engagedMembers ||
          b.views - a.views
      ),
    [impact.discussions]
  );
        ...
        <section className="grid grid-cols-2 border-b border-[var(--loombus-border)] sm:grid-cols-4">
          {[
            ...
            <div
              key={String(label)}
              className={`py-5 ${
                index > 0 ? "border-l border-[var(--loombus-border)] pl-4" : ""
              }`}
            >
          ...
          <section aria-label="Discussion insights">
            <div className="grid grid-cols-2 border-b border-[var(--loombus-border)] sm:grid-cols-5">
              {[
                ...
                >
                  <span className="block text-xs text-[var(--loombus-text-muted)]">
                    {label}
                  </span>
            ...
            <div className="border-b border-[var(--loombus-border)] py-5">
              <div className="flex flex-wrap items-start justify-between gap-5">
                <div className="max-w-2xl">
                  ...
                  <p className="mt-1 text-sm leading-6 text-[var(--loombus-text-muted)]">
                    Signal actions are replies received plus saves earned. Signal contributors are distinct members who replied or saved. Views and reach do not increase Signal.
                  </p>
                </div>
              ...
              <div className="mt-5 grid grid-cols-3 border-t border-[var(--loombus-border)]">
                <div className="py-4">
                  <span className="block text-xs text-[var(--loombus-text-muted)]">Signal actions</span>
                  <strong className="mt-1 block text-xl font-black">
                    {loadingSummary ? "—" : impact.totals.signalActions.toLocaleString()}
                  </strong>
                </div>
                <div className="border-l border-[var(--loombus-border)] py-4 pl-4">
                  <span className="block text-xs text-[var(--loombus-text-muted)]">Signal contributors</span>
                  <strong className="mt-1 block text-xl font-black">
                    {loadingSummary ? "—" : impact.totals.signalContributors.toLocaleString()}
                  </strong>
                </div>
                <div className="border-l border-[var(--loombus-border)] py-4 pl-4">
                  <span className="block text-xs text-[var(--loombus-text-muted)]">Signal depth</span>
                  <strong className="mt-1 block text-xl font-black">
                    {loadingSummary ? "—" : formatDepth(impact.totals.signalDepth)}
                  </strong>
            ...
            <div className="border-b border-[var(--loombus-border)] py-5">
              <div className="mb-3 flex items-end justify-between gap-4">
                <div>
                  ...
                  <h2 className="mt-2 text-xl font-black">Where your signal is forming.</h2>
                </div>
                <span className="text-xs text-[var(--loombus-text-muted)]">{rangeLabel}</span>
              </div>

              {rankedDiscussions.length ? (
                rankedDiscussions.map((item) => (
                  <Link
                    key={item.id}
                    href={`/discussions/${item.id}`}
                    className="grid gap-2 border-t border-[var(--loombus-border)] py-4 sm:grid-cols-[minmax(0,1fr)_repeat(6,88px)] sm:items-center"
                  >
                    ...
                    {[
                      ["Views", item.views],
                      ["Reach", item.uniqueReach],
                      ["Replies", item.repliesReceived],
                      ["Saves", item.savesEarned],
                      ["Signal", item.signalActions],
                      ["Contrib.", item.signalContributors],
                    ].map(([label, value]) => (
                      <span
                        key={String(label)}
                        className="text-xs text-[var(--loombus-text-muted)] sm:text-right"
                      >
                        <b className="text-[var(--loombus-text)]">
                ...
                <p className="border-t border-[var(--loombus-border)] py-4 text-sm text-[var(--loombus-text-muted)]">
                  No discussion activity recorded in this period.
                </p>
              )}
            </div>
              ...
              <div>
                {replies.map((reply) => (
                  ...
                  >
                    ...
                    <span className="shrink-0 text-xs font-bold text-[var(--loombus-text-muted)]">
                      Open
                    </span>
                  </Link>
                ))}
