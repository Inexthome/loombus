...
export default function LibraryPublishedRevisionsPage() {
  ...
  const loadRevision = useCallback(async (publicationId: string | null) => {
    ...
    const [versionResult, sourceResult] = await Promise.all([
      supabase
        ...
        .select("id,publication_id,version_number,title,subtitle,description,publication_type,author_name,publisher_name,language_code,isbn,version_status")
  ...
  const loadWorkspace = useCallback(async () => {
    ...
    const result = await supabase
      ...
      .select("id,title,subtitle,publication_type,author_name,publisher_name,language_code,isbn,status")
      .in("id", ids)
      .eq("status", "published")
      .order("title");
    ...
    const nextId = selectedId && rows.some((row) => row.id === selectedId)
      ? selectedId
      : rows[0]?.id ?? null;
  ...
  async function createRevision() {
    ...
    if (result.error) {
      setError("Unable to start a revision. Confirm there is no existing open revision.");
    } else {
  ...
  async function saveRevision() {
    ...
    setError(null);
    setMessage(null);
    const result = await supabase.rpc("update_library_author_revision", {
      p_version_id: version.id,
      ...payload(form),
    });
    if (result.error) {
      setError("Unable to save this revision in its current review state.");
    } else {
      setMessage("Revision metadata saved.");
  ...
  async function uploadAndProcess() {
    ...
    try {
      ...
      setMessage(`Revision EPUB processed${body?.sectionCount ? ` into ${body.sectionCount} readable sections` : ""}.`);
      setFile(null);
      await loadRevision(selectedId);
    } catch (cause) {
  ...
  async function submitRevision() {
    ...
    const save = await supabase.rpc("update_library_author_revision", {
      p_version_id: version.id,
      ...payload(form),
    });
    if (save.error) {
    ...
    if (result.error) {
      setError("Unable to submit this revision. Confirm the replacement EPUB is processed and ready.");
    } else {
    ...
    <main data-library-publish-editorial className="library-publish-page">
      <div className="library-publish-shell">
        <div className="library-publish-topbar">
          <Link href="/library/publish" className="library-publish-back">
        ...
        <header className="library-publish-header">
          ...
          <p>
            Stage a new version privately, update its metadata and EPUB, and submit it for Loombus review. The current published version remains readable until an approved revision is explicitly published.
          </p>
        </header>
        ...
        <div className="library-publish-workspace">
          <aside className="library-publish-rail" aria-labelledby="library-revisions-list-heading">
            <div className="library-publish-rail-heading">
              <div>
                <p className="library-publish-eyebrow">Your live catalog</p>
                <h2 id="library-revisions-list-heading">Published work</h2>
              </div>
              {loading ? <Loader2 className="library-publish-spinner" aria-label="Loading published work" /> : null}
            </div>

            {!loading && !publications.length ? (
              <p className="library-publish-empty">No currently published author works are available to revise.</p>
            ) : null}
              ...
              {publications.map((row) => (
                ...
                >
                  ...
                  <span className="library-publish-publication-meta">
                    <span className="library-publish-status">published</span>
                    <span className="library-publish-type">{row.publication_type}</span>
                  </span>
                </button>
              ))}
            </div>
            ...
            {!selected ? (
              ...
              <>
                <header className="library-publish-editor-header">
                  <div>
                    <p className="library-publish-eyebrow">Published</p>
                    <h2>{selected.title}</h2>
                  </div>
                  <span className="library-publish-state">Live version protected</span>
                </header>

                <p className="library-publish-lock-note">
                  No open revision is staged. Starting one creates a private version snapshot; it does not modify or unpublish the edition readers can currently access.
                </p>
                ...
                <footer className="library-publish-actions">
                  ...
                  >
                    {working ? <Loader2 className="library-publish-spinner" aria-hidden="true" /> : <Sparkles aria-hidden="true" />}
                    Create revision
                  </button>
                </footer>
              </>
              ...
              <>
                <header className="library-publish-editor-header">
                  <div>
                    <p className="library-publish-eyebrow">
                      Version {version.version_number} · {reviewStatusLabel(review.submission_status)}
                    </p>
                    <h2>Staged revision</h2>
                  </div>
                  {ready ? <span className="library-publish-state">EPUB ready</span> : <span className="library-publish-state">Live version protected</span>}
                </header>
                ...
                <LibraryBibliographicMetadataEditor
                  ...
                  editable={Boolean(editable && !working)}
                />
                ...
                <section className="library-publish-commerce" aria-labelledby="library-revision-epub-heading">
                  <div className="library-publish-commerce-heading">
                    <div>
                      <p className="library-publish-eyebrow">Replacement EPUB</p>
                      <h3 id="library-revision-epub-heading">Stage the content for Version {version.version_number}.</h3>
                    </div>
                  </div>
                  <p className="library-publish-commerce-copy">
                    The live edition remains untouched. Loombus keeps the uploaded EPUB private and writes normalized readable sections only to this staged revision.
                  </p>

                  {source ? (
                    <p className="library-publish-lock-note">
                      Processing status: <strong>{source.ingestion_status}</strong>
                      {source.ingestion_error ? ` · ${source.ingestion_error}` : ""}
                    </p>
                  ) : null}
                  ...
                  {editable ? (
                    <div className="library-publish-commerce-fields">
                      <label className="library-publish-field">
                        <span className="library-publish-field-label">EPUB file <span>Maximum 50 MiB</span></span>
                        <input
                          type="file"
                      ...
                      >
                        {working ? (
                          <Loader2 className="library-publish-spinner" aria-hidden="true" />
                        ) : source ? (
                          <RefreshCw aria-hidden="true" />
                        ) : (
                          <FileUp aria-hidden="true" />
                        )}
                        {source ? "Replace & process" : "Upload & process"}
                      </button>
                    </div>
                  ) : null}
                </section>
                ...
                {!editable ? (
                  <p className="library-publish-lock-note">
                    This revision is locked in its current editorial state. The live publication remains unchanged while Loombus review controls the next transition.
                  </p>
                ) : null}
                  ...
                  <div className="library-publish-secondary-actions">
                    {editable ? (
                      <button
                        ...
                        className="library-publish-secondary"
                      >
                        {working ? <Loader2 className="library-publish-spinner" aria-hidden="true" /> : <Save aria-hidden="true" />}
                        Save revision
                      </button>
                    ) : null}
                    <LibraryVersionNormalizedPreview versionId={version.id} disabled={!ready || working} />
                  </div>
