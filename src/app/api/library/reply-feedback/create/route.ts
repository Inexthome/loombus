...
export async function POST(request: NextRequest) {
  try {
    ...
    const replyId = text(body.replyId);
    ...
    const replyBodySha256 = text(body.replyBodySha256);

    if (!discussionId || !replyId || !["claim", "knowledge"].includes(kind)) {
      return NextResponse.json({ error: "Reply feedback request is incomplete." }, { status: 400 });
    }
    ...
    if (!/^[0-9a-f]{64}$/.test(replyBodySha256)) {
      return NextResponse.json({ error: "Reply text version is invalid." }, { status: 400 });
    }

    const [{ data: discussion, error: discussionError }, { data: reply, error: replyError }] = await Promise.all([
      supabase
        .from("discussions")
      ...
      supabase
        .from("replies")
        .select("id, discussion_id, user_id, body")
        .eq("id", replyId)
    ...
    if (discussionError || !discussion || replyError || !reply) {
      return NextResponse.json({ error: "This reply is unavailable." }, { status: 404 });
    }

    const canonicalBody = String(reply.body ?? "");
    const canonicalHash = sha256(canonicalBody);
    if (canonicalHash !== replyBodySha256) {
      return NextResponse.json({ error: "The reply changed after you selected this text. Select it again." }, { status: 409 });
    }
    if (endOffset > canonicalBody.length || canonicalBody.slice(startOffset, endOffset) !== selectedText) {
      return NextResponse.json({ error: "The selected text no longer matches the reply. Select it again." }, { status: 409 });
    }

    const userId = authData.user.id;
    const sourceTitle = String(discussion.title ?? "").trim();
    const sourceTopic = String(discussion.topic ?? "").trim();
    const replyAuthorId = String(reply.user_id);
    ...
    if (kind === "claim") {
      ...
      const { data: claim, error: claimError } = await supabase
        ...
        .insert({ user_id: userId, statement, claim_type: claimType, status, rationale: rationale || null })
      ...
      if (provenanceError) {
        ...
        return NextResponse.json({ error: "The claim was not saved because its reply provenance could not be recorded." }, { status: 500 });
      }
    ...
    const { data: knowledge, error: knowledgeError } = await supabase
      ...
      .insert({ user_id: userId, title, summary: summary || null, knowledge_type: knowledgeType, status })
    ...
    if (provenanceError) {
      ...
      return NextResponse.json({ error: "The knowledge object was not saved because its reply provenance could not be recorded." }, { status: 500 });
    }

    return NextResponse.json({ kind: "knowledge", id: knowledge.id });
  } catch (error) {
    console.error("Library reply feedback create failed:", error);
