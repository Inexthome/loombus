...
function errorResponse(error: unknown) {
  ...
  return NextResponse.json(
    { error: "Room moderation could not be completed." },
...
export async function GET(request: NextRequest, context: RouteContext) {
  ...
  if (!validUuid(roomId)) return NextResponse.json({ error: "Invalid Room id." }, { status: 400 });
  try {
    return NextResponse.json(await getRoomModerationOverview(roomId, auth.userId), {
...
export async function POST(request: NextRequest, context: RouteContext) {
  ...
  try {
    return NextResponse.json(
      await performRoomModerationAction(roomId, auth.userId, action, body ?? {}),
      { headers: { "Cache-Control": "private, no-store" } }
    );
  } catch (error) {
    return errorResponse(error);
