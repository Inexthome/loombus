...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!serviceKey) {
      return NextResponse.json({ ok: false, locked: true, reason: "Shadow record endpoint is not configured." });
    }
    ...
    if (flagError) {
      return NextResponse.json({ ok: false, locked: true, reason: "V2 access could not be verified." });
    }
    ...
    if (draftError) {
      return NextResponse.json({ ok: false, locked: true, reason: "Draft could not be loaded." });
    }
      ...
      return NextResponse.json(
        {
          ...
          locked: true,
          status: "needs_work",
          reason: "Draft must pass server validation before a shadow record can be created.",
          checks,
        },
        { status: 400 }
      );
    ...
    const mode = VALID_MODES.has(draft.mode ?? "") ? draft.mode : "open_discussion";

    const { data: shadowRecord, error: shadowError } = await adminSupabase
      ...
      .insert({
        ...
        source: "v2_create_readiness",
      })
      .select("id, status, created_at, title, topic, mode, tags")
      .single();

    if (shadowError) {
      return NextResponse.json({ ok: false, locked: true, reason: "Shadow record storage is not configured." });
    }
    ...
    return NextResponse.json({
      ...
      locked: true,
      status: "shadow_recorded",
      reason: "Non-public shadow record created. Final action remains locked.",
      shadowRecord,
    });
  } catch (error) {
    console.error("Unexpected V2 shadow record failure:", error);
    return NextResponse.json({ ok: false, locked: true, reason: "Unexpected shadow record failure." }, { status: 500 });
  }
}
