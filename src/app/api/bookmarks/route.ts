...
export async function POST(request: NextRequest) {
  ...
  const { data: existingBookmark, error: existingError } = await supabase
    ...
    .maybeSingle();
  if (existingError) return jsonError("Unable to verify saved discussion.", 400);
  if (existingBookmark) return NextResponse.json({ bookmark: existingBookmark, existing: true });

  let unlimitedOrganization = false;
  try { unlimitedOrganization = await hasUnlimitedOrganization(accountAccess.user.id); }
  catch { return jsonError("Unable to verify Saved access.", 503); }
  ...
  if (!unlimitedOrganization) {
    const { count, error: countError } = await service.from("bookmarks").select("id", { count: "exact", head: true }).eq("user_id", accountAccess.user.id);
    if (countError) return jsonError("Unable to verify Saved usage.", 400);
    const limit = ORGANIZATION_LIMITS.free.saves;
    if ((count ?? 0) >= limit) return jsonError(`Free Saved is limited to ${limit} discussions. Upgrade to Premium for unlimited saves.`, 403, "organization_limit_reached", { limit });
  }

  const { data: bookmark, error } = await service.from("bookmarks").insert({ user_id: accountAccess.user.id, discussion_id: discussionId }).select("id").single();
  if (error) {
    const message = error.message || "Already saved or unable to save.";
    if (message.includes("Free Saved is limited")) return jsonError(message, 403, "organization_limit_reached", { limit: ORGANIZATION_LIMITS.free.saves });
...
export async function DELETE(request: NextRequest) {
  ...
  let query = service.from("bookmarks").delete().eq("user_id", accountAccess.user.id);
  if (isValidUuid(bookmarkId)) query = query.eq("id", bookmarkId);
  else if (isValidUuid(discussionId)) query = query.eq("discussion_id", discussionId);
  else return jsonError("Missing bookmark id or discussion id.", 400);

  const { error } = await query;
  if (error) return jsonError(error.message || "Unable to remove saved discussion.", 400);
