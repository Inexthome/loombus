...
export async function GET(request: NextRequest) {
  ...
  if (!viewerId) return jsonError("Attachment not found.", 404);

  const { data: viewerProfile, error: profileError } = await service
    .from("profiles")
    .select("is_admin, account_status, enforcement_reason, suspended_until")
    .eq("id", viewerId)
    .maybeSingle();
  if (profileError) return jsonError("Unable to verify account standing.", 503);
  const enforcement = getAccountEnforcementResult((viewerProfile ?? null) as ProfileAccess | null);
  if (!enforcement.allowed) return jsonError("Attachment not found.", 404);
  ...
  if (signedError || !signed?.signedUrl) return jsonError("Unable to deliver attachment.", 503);

  return NextResponse.json(
    { url: signed.signedUrl, expiresIn: SIGNED_URL_TTL_SECONDS, protected: true },
