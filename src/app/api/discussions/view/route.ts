...
import { getMemberPrivacy } from "@/lib/member-privacy-server";
...
async function getViewerId(request: NextRequest) {
  ...
  const { data } = await requestSupabase.auth.getUser();
...
async function getDiscussionMetrics(
  ...
  const [{ count: replyCount }, { count: saveCount }, { count: viewCount }, { count: stickyCount }] =
    await Promise.all([
      supabase
        .from("replies")
...
export async function POST(request: NextRequest) {
  ...
  if (!supabase) return jsonError("Discussion view service is not configured.", 503);
  ...
  if (!audienceAccess.allowed) return jsonError("Discussion not found.", 404);

  const dedupeSince = new Date(
    Date.now() - VIEW_DEDUPE_HOURS * 60 * 60 * 1000
  ).toISOString();
  let shouldTrackView = Boolean(viewerId && viewerId !== discussion.user_id);
  ...
  if (shouldTrackView && viewerId) {
    const { data: recentView, error: recentViewError } = await supabase
      ...
      .limit(1)
      .maybeSingle();
    if (recentViewError) return jsonError(recentViewError.message, 500);
    shouldTrackView = !recentView;
  }

  if (shouldTrackView && viewerId) {
    const viewerPrivacy = await getMemberPrivacy(supabase, viewerId);
  ...
  return NextResponse.json(
    { tracked: shouldTrackView, discussionId, ...metrics },
