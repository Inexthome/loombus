...
export type RoomWorkspaceSection =
  | "discussions"
  | "announcements"
  | "events"
  | "members";
...
export default function RoomWorkspaceSectionClient({
  section,
}: {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel(`room-section:${section}:${roomId}`)
      .on(
        "postgres_changes",
  ...
  }, [access?.allowed, loadWorkspace, roomId, section]);
      ...
      <div className="rooms-live-shell">
        ...
        {section === "announcements" ? (
          <section className="room-workspace-panel">
            ...
            {access.canManage ? (
              <form onSubmit={createAnnouncement} className="room-workspace-management-form">
                  ...
                  <article key={member.id} className="room-workspace-member">
                    ...
                    <div className="room-workspace-member-controls">
                      {access.canManage && member.role !== "owner" ? (
                        ...
                        <span className="rooms-live-role-badge">{roleLabel(member.role)}</span>
                      )}
                      {access.canManage && member.role !== "owner" ? (
                        <button
                          type="button"
                          onClick={() => {
                            if (!window.confirm(`Remove ${getProfileDisplayName(member.profile)} from this Room?`)) return;
                            void submitAction(
                              "remove_member",
                              { memberId: member.id },
                              `remove:${member.id}`,
                ...
                {pendingApplications.length > 0 ? (
                  <div className="room-workspace-application-list">
                    {pendingApplications.map((application) => (
                      <article key={application.id} className="room-workspace-application">
                        <div className="room-workspace-author-row">
                          <ProfileAvatar profile={application.applicant} size="md" />
                          <div>
                            <strong>{getProfileDisplayName(application.applicant)}</strong>
                            <span>Requested {formatRelativeTime(application.createdAt)}</span>
                          </div>
                        </div>
...
function AnnouncementCard({
  ...
  return (
    <article className={`room-workspace-announcement ${priorityClass(announcement.priority)}`}>
      ...
      <small>
        {getProfileDisplayName(announcement.creator)} · {formatRelativeTime(announcement.createdAt)}
      </small>
    </article>
  );
}
