...
export default function RoomRouteFrameV3({ children }: { children: ReactNode }) {
  ...
  const roomTools: NavItem[] = [
    ...
    ...(STUDIO_PLAN_KEYS.has(plan.id)
      ? [{ kind: "feature" as const, id: "workspace:studio", label: "Room Studio", Icon: Wrench, feature: { id: "workspace:studio", kind: "studio" as const, label: "Room Studio" } }]
      : []),
    ...(ORGANIZATION_PLAN_KEYS.has(plan.id)
      ? [{ kind: "feature" as const, id: "workspace:organization", label: "Organization Console", Icon: Building2, feature: { id: "workspace:organization", kind: "organization" as const, label: "Organization Console" } }]
      : []),
  ...
  if (payload.access.canManage) {
    managementRoutes.push({ kind: "route", id: "route:governance", href: `${roomBase}/governance`, label: "Ownership & Governance", Icon: ShieldCheck, badge: payload.metrics.pendingApplications });
    managementRoutes.push({ kind: "route", id: "route:age-safety", href: `${roomBase}/age-safety`, label: "Minor safety", Icon: ShieldCheck });
  }
  if (payload.access.isOwner) {
    managementRoutes.push({ kind: "route", id: "route:retention", href: `${roomBase}/retention`, label: "Retention", Icon: FileClock });
    managementRoutes.push({ kind: "route", id: "route:billing", href: `${roomBase}/billing`, label: "Billing", Icon: CreditCard });
  }
  ...
  return (
    <div className="rooms-phase1-shell">
      ...
      <aside className="rooms-phase1-sidebar" data-open={sidebarOpen ? "true" : "false"} aria-label="Room navigation">
      ...
      <button type="button" className="rooms-phase1-sidebar-backdrop" data-open={sidebarOpen ? "true" : "false"} aria-label="Close Room navigation" onClick={() => setSidebarOpen(false)} />
      <div className="rooms-phase1-main-column">
        <header className="rooms-phase1-room-header">
          <div className="rooms-phase1-room-header-main">
            <button type="button" className="rooms-phase1-menu-button" aria-label={sidebarOpen ? "Close Room navigation" : "Open Room navigation"} aria-expanded={sidebarOpen} onClick={() => setSidebarOpen((current) => !current)}>{sidebarOpen ? <X aria-hidden="true" /> : <Menu aria-hidden="true" />}</button>
