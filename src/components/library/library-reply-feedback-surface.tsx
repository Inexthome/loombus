...
import { ArrowLeft, Check, FlaskConical, Loader2, MessageCircle } from "lucide-react";
import { useSearchParams } from "next/navigation";
import { useEffect, useMemo, useRef, useState } from "react";
import { supabase } from "@/lib/supabase/client";

type Discussion = { id: string; title: string; topic: string };
type ReplyRow = { id: string; user_id: string; body: string; created_at: string };
type SelectionState = { text: string; start: number; end: number };
...
export function LibraryReplyFeedbackSurface() {
  ...
  const [replyHash, setReplyHash] = useState("");
  ...
  const selectedReply = useMemo(() => replies.find((reply) => reply.id === replyId) ?? null, [replies, replyId]);
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      setError(null);
      if (!discussionId) {
        setError("Choose a discussion before building Library knowledge from a reply.");
        setLoading(false);
        return;
      }

      const [discussionResult, replyResult] = await Promise.all([
        supabase.from("discussions").select("id, title, topic").eq("id", discussionId).is("deleted_at", null).single(),
        supabase.from("replies").select("id, user_id, body, created_at").eq("discussion_id", discussionId).is("deleted_at", null).order("created_at", { ascending: true }),
      ]);
      ...
      setDiscussion(discussionResult.data as Discussion);
      const visibleReplies = (replyResult.data ?? []) as ReplyRow[];
      setReplies(visibleReplies);
      setReplyId(visibleReplies[0]?.id ?? "");
      setLoading(false);
    }
    void load();
  ...
  useEffect(() => {
    ...
    async function updateHash() {
      ...
      const hash = await sha256(selectedReply.body ?? "");
      if (!cancelled) setReplyHash(hash);
    }
    void updateHash();
    return () => { cancelled = true; };
  }, [selectedReply]);
  ...
  async function save() {
    if (!discussion || !selectedReply || !selection || !selectionValid || !replyHash) return;
    setSaving(true);
    setError(null);
    setSavedKind(null);
    ...
    const payload = mode === "claim"
      ? { ...base, kind: "claim", statement, claimType, status: claimStatus, rationale }
      : { ...base, kind: "knowledge", title, summary, knowledgeType, status: knowledgeStatus };

    const response = await fetch("/api/library/reply-feedback/create", {
      method: "POST",
    ...
    if (!response.ok) {
      setError(result.error ?? "Unable to save this reply selection to Library Research.");
      setSaving(false);
      return;
    }
    setSavedKind(mode);
    ...
    <main className="min-h-screen bg-[var(--loombus-page-bg)] px-4 pb-24 pt-6 text-[var(--loombus-text)] sm:px-6 md:pt-20">
      <div className="mx-auto max-w-5xl">
        <header className="border-b border-[var(--loombus-border)] pb-6">
          <Link href={discussion ? `/discussions/${discussion.id}` : "/discussions"} className="inline-flex items-center gap-2 text-sm font-bold text-[var(--loombus-gold)]"><ArrowLeft className="size-4" /> Discussion</Link>
          <div className="mt-5 flex items-start gap-3">
            <div className="grid size-11 place-items-center rounded-2xl bg-[var(--loombus-gold-surface)] text-[var(--loombus-gold)]"><MessageCircle className="size-5" /></div>
            <div>
              <h1 className="text-3xl font-black tracking-tight">Reply → Knowledge</h1>
              <p className="mt-1 max-w-2xl text-sm text-[var(--loombus-text-muted)]">Choose a visible reply, select an exact passage, and turn it into a new private claim or knowledge object. The reply and public discussion stay unchanged.</p>
            </div>
          </div>
        </header>
        ...
        {discussion ? (
          <div className="mt-6 grid gap-6 lg:grid-cols-[minmax(0,1.15fr)_minmax(340px,0.85fr)]">
            <section className="rounded-[1.5rem] border border-[var(--loombus-border)] bg-[var(--loombus-surface)] p-5">
              ...
              {replies.length ? (
                <>
                  <label className="mt-4 block text-xs font-black">Visible reply
                    <select value={replyId} onChange={(event) => setReplyId(event.target.value)} className="mt-1.5 w-full rounded-xl border border-[var(--loombus-border)] bg-[var(--loombus-surface)] p-2.5 text-sm font-normal">
                      {replies.map((reply, index) => <option key={reply.id} value={reply.id}>Reply {index + 1} · {reply.body.slice(0, 70).replace(/\s+/g, " ")}{reply.body.length > 70 ? "…" : ""}</option>)}
                    </select>
                  </label>
                  <p className="mt-3 text-xs text-[var(--loombus-text-muted)]">Select 20–4000 characters from the canonical reply body below.</p>
                  <textarea ref={textareaRef} readOnly value={selectedReply?.body ?? ""} onSelect={captureSelection} rows={20} className="mt-4 w-full resize-y rounded-2xl border border-[var(--loombus-border)] bg-[var(--loombus-page-bg)] p-4 text-sm leading-7 outline-none selection:bg-[var(--loombus-gold)] selection:text-black" aria-label="Selected discussion reply text" />
              ...
              ) : <div className="mt-5 rounded-2xl bg-[var(--loombus-surface-strong)] p-5 text-sm text-[var(--loombus-text-muted)]">No visible replies are available yet.</div>}
            </section>
            ...
            <section className="rounded-[1.5rem] border border-[var(--loombus-border)] bg-[var(--loombus-surface)] p-5">
              ...
              <button type="button" disabled={!selectionValid || saving || (mode === "claim" ? !statement.trim() : !title.trim())} onClick={() => void save()} className="mt-5 inline-flex w-full items-center justify-center gap-2 rounded-full bg-[var(--loombus-gold)] px-4 py-3 text-sm font-black text-black disabled:opacity-40">
                {saving ? <Loader2 className="size-4 animate-spin" /> : <FlaskConical className="size-4" />} Save private {mode}
              </button>

              {savedKind ? <div className="mt-4 rounded-2xl border border-[var(--loombus-border)] bg-[var(--loombus-gold-surface)] p-4 text-sm"><div className="flex items-center gap-2 font-black text-[var(--loombus-gold)]"><Check className="size-4" /> Saved to private Library Research</div><Link href="/library/research/evidence" className="mt-2 inline-block text-xs font-black text-[var(--loombus-gold)]">Open Evidence & Knowledge →</Link></div> : null}
            </section>
          </div>
        ) : null}
      </div>
