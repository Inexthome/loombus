...
export function LibraryFunctionalSurface() {
  ...
  const loadLibrary = useCallback(async () => {
    ...
    const publicationResult = await supabase
      ...
      .select("id, slug, title, subtitle, description, publication_type, author_name, publisher_name, cover_url, publication_date")
      .order("publication_date", { ascending: false, nullsFirst: false })
      .order("title", { ascending: true });

    if (publicationResult.error) {
      setErrorMessage("Unable to load Library publications.");
      setLoading(false);
      return;
    }
  ...
  const filteredPublications = useMemo(
    () => publications.filter((publication) => publicationMatches(publication, normalizedQuery)),
    [publications, normalizedQuery]
  );

  const myLibraryPublications = useMemo(
    () => memberItems.map((item) => publicationById.get(item.publication_id)).filter((publication): publication is Publication => Boolean(publication)).filter((publication) => publicationMatches(publication, normalizedQuery)),
  ...
  const authorNames = useMemo(() => {
    ...
    filteredPublications.forEach((publication) => {
      if (publication.author_name?.trim()) names.add(publication.author_name.trim());
    });
    ...
    return (
      <article className="rounded-[1.5rem] border border-[var(--loombus-border)] bg-[var(--loombus-surface-strong)] p-5">
        <div className="flex items-start justify-between gap-4">
          <span className="grid h-12 w-12 shrink-0 place-items-center rounded-2xl border border-[color:color-mix(in_srgb,var(--loombus-gold)_35%,var(--loombus-border))] bg-[var(--loombus-gold-surface)] text-[var(--loombus-gold)]">
            <BookOpen className="h-5 w-5" aria-hidden="true" />
          </span>
          <span className="rounded-full border border-[var(--loombus-border)] px-2.5 py-1 text-[10px] font-bold uppercase tracking-[0.12em] text-[var(--loombus-text-muted)]">
            {publication.publication_type}
          </span>
        </div>
        ...
        <button
          ...
          onClick={() => void toggleMyLibrary(publication.id)}
          className="mt-5 inline-flex items-center gap-2 rounded-full border border-[var(--loombus-border)] px-3.5 py-2 text-xs font-semibold text-[var(--loombus-text)] transition hover:border-[var(--loombus-gold)] disabled:cursor-wait disabled:opacity-60"
        >
      ...
      <div className="mx-auto w-full max-w-6xl">
        <section className="overflow-hidden rounded-[2rem] border border-[var(--loombus-border)] bg-[var(--loombus-surface)] shadow-sm">
            ...
            <div className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
              <div className="max-w-2xl">
                ...
                <p className="mt-3 max-w-xl text-sm leading-6 text-[var(--loombus-text-muted)] sm:text-base">
                  Discover published long-form work and keep your personal reading state private by default.
                </p>
              </div>
          ...
          <div className="grid gap-4 p-5 sm:grid-cols-3 sm:p-8">
            ...
            ].map(({ title, count, icon: Icon, tab }) => (
              <button key={title} type="button" onClick={() => setActiveTab(tab)} className="rounded-[1.5rem] border border-[var(--loombus-border)] bg-[var(--loombus-surface-strong)] p-5 text-left transition hover:border-[var(--loombus-gold)]">
                ...
                <p className="mt-2 text-2xl font-semibold">{loading ? "—" : count}</p>
                <span className="mt-4 inline-flex items-center gap-1 text-sm font-semibold text-[var(--loombus-gold)]">Open <ChevronRight className="h-4 w-4" aria-hidden="true" /></span>
              </button>
            ))}
          </div>
        </section>
        ...
        <section className="mt-6 rounded-[2rem] border border-[var(--loombus-border)] bg-[var(--loombus-surface)] p-5 sm:p-8">
          <div className="flex items-center justify-between gap-4">
            <div>
              <p className="text-xs font-bold uppercase tracking-[0.2em] text-[var(--loombus-gold)]">{activeTab}</p>
              <h2 className="mt-2 text-xl font-semibold">{activeTab === "Discover" ? "Published Library" : activeTab}</h2>
            </div>
            {loading ? <Loader2 className="h-5 w-5 animate-spin text-[var(--loombus-gold)]" aria-label="Loading Library" /> : null}
          </div>

          {errorMessage ? <p className="mt-5 rounded-2xl border border-[var(--loombus-border)] bg-[var(--loombus-surface-strong)] px-4 py-3 text-sm text-[var(--loombus-text-muted)]">{errorMessage}</p> : null}

          {!loading && activeTab === "Discover" ? (
            filteredPublications.length ? <div className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">{filteredPublications.map((publication) => <PublicationCard key={publication.id} publication={publication} />)}</div> : <div className="mt-6"><EmptyState title="No published matches" body="Try a different title, author, publisher, or topic." /></div>
          ) : null}

          {!loading && activeTab === "My Library" ? (
            myLibraryPublications.length ? <div className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">{myLibraryPublications.map((publication) => <PublicationCard key={publication.id} publication={publication} />)}</div> : <div className="mt-6"><EmptyState title="My Library is empty" body="Add a published work from Discover to keep it in your private Library." /></div>
          ) : null}

          {!loading && activeTab === "Continue Reading" ? (
            continueReadingRows.length ? (
              <div className="mt-6 space-y-3">
                {continueReadingRows.map((row) => {
                  const publication = publicationById.get(row.publication_id)!;
                  return (
                    <article key={row.publication_id} className="rounded-[1.5rem] border border-[var(--loombus-border)] bg-[var(--loombus-surface-strong)] p-5">
                      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                        <div>
                          <h3 className="font-semibold">{publication.title}</h3>
                          <p className="mt-1 text-sm text-[var(--loombus-text-muted)]">{row.locator ?? "Saved reading position"}</p>
                        </div>
                        <span className="text-sm font-semibold text-[var(--loombus-gold)]">{Number(row.progress_percent).toFixed(0)}%</span>
                      </div>
                      <div className="mt-4 h-2 overflow-hidden rounded-full bg-[var(--loombus-surface-muted)]">
                        <div className="h-full rounded-full bg-[var(--loombus-gold)]" style={{ width: `${Math.min(100, Math.max(0, Number(row.progress_percent)))}%` }} />
                      </div>
                    </article>
                  );
                })}
              </div>
            ) : <div className="mt-6"><EmptyState title="Nothing in progress yet" body="Reading progress will appear here once the Loombus Reader begins saving your position." /></div>
          ) : null}
            ...
            highlights.length || notes.length ? (
              <div className="mt-6 space-y-4">
                {highlights.map((highlight) => {
                  ...
                  const attachedNotes = notes.filter((note) => note.highlight_id === highlight.id);
                  return (
                    <article key={highlight.id} className="rounded-[1.5rem] border border-[var(--loombus-border)] bg-[var(--loombus-surface-strong)] p-5">
                      <div className="flex items-center gap-2 text-xs font-bold uppercase tracking-[0.14em] text-[var(--loombus-gold)]"><Highlighter className="h-3.5 w-3.5" aria-hidden="true" />Highlight</div>
                      <p className="mt-3 text-base leading-7">{highlight.selected_text}</p>
                      <p className="mt-3 text-xs text-[var(--loombus-text-muted)]">{publication?.title ?? "Publication"} · {highlight.locator}</p>
                      {attachedNotes.map((note) => <div key={note.id} className="mt-4 rounded-2xl border border-[var(--loombus-border)] bg-[var(--loombus-surface)] p-4"><div className="flex items-center gap-2 text-xs font-semibold text-[var(--loombus-text-muted)]"><Bookmark className="h-3.5 w-3.5 text-[var(--loombus-gold)]" aria-hidden="true" />Private note</div><p className="mt-2 text-sm leading-6">{note.body}</p></div>)}
                    </article>
                  );
                })}
                {notes.filter((note) => !note.highlight_id).map((note) => <article key={note.id} className="rounded-[1.5rem] border border-[var(--loombus-border)] bg-[var(--loombus-surface-strong)] p-5"><div className="flex items-center gap-2 text-xs font-bold uppercase tracking-[0.14em] text-[var(--loombus-gold)]"><Bookmark className="h-3.5 w-3.5" aria-hidden="true" />Private note</div><p className="mt-3 text-sm leading-6">{note.body}</p><p className="mt-3 text-xs text-[var(--loombus-text-muted)]">{publicationById.get(note.publication_id)?.title ?? "Publication"}{note.locator ? ` · ${note.locator}` : ""}</p></article>)}
              </div>
            ) : <div className="mt-6"><EmptyState title="No highlights or notes yet" body="Private annotations created in the Loombus Reader will collect here." /></div>
          ) : null}

          {!loading && activeTab === "Authors" ? (
            authorNames.length ? <div className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">{authorNames.map((author) => <article key={author} className="rounded-[1.5rem] border border-[var(--loombus-border)] bg-[var(--loombus-surface-strong)] p-5"><span className="grid h-11 w-11 place-items-center rounded-2xl bg-[var(--loombus-gold-surface)] text-[var(--loombus-gold)]"><UserRound className="h-5 w-5" aria-hidden="true" /></span><h3 className="mt-4 font-semibold">{author}</h3><p className="mt-1 text-sm text-[var(--loombus-text-muted)]">{publications.filter((publication) => publication.author_name?.trim() === author).length} published {publications.filter((publication) => publication.author_name?.trim() === author).length === 1 ? "work" : "works"}</p></article>)}</div> : <div className="mt-6"><EmptyState title="No author matches" body="Authors appear here when published Library metadata includes an author name." /></div>
          ) : null}
        </section>
      </div>
    </main>
