...
export function LibraryDiscoverCatalog({ query, savedIds, mutationId, onToggleSaved }: Props) {
  ...
  return (
    <div>
      <div className="mb-6 flex flex-wrap items-end justify-between gap-3 border-b border-[var(--loombus-border)] pb-5">
        <div className="flex flex-wrap items-end gap-3">
          <label className="grid gap-1.5 text-xs font-semibold text-[var(--loombus-text-muted)]">
            Type
            <select value={publicationType} onChange={(event) => setPublicationType(event.target.value as PublicationType)} className="min-h-10 rounded-xl border border-[var(--loombus-border)] bg-[var(--loombus-surface)] px-3 text-sm text-[var(--loombus-text)] outline-none focus:border-[var(--loombus-gold)]">
              {publicationTypes.map((type) => <option key={type} value={type}>{type === "all" ? "All types" : humanType(type)}</option>)}
            </select>
          </label>
          <label className="grid gap-1.5 text-xs font-semibold text-[var(--loombus-text-muted)]">
            Sort
            <select value={sort} onChange={(event) => setSort(event.target.value as SortMode)} className="min-h-10 rounded-xl border border-[var(--loombus-border)] bg-[var(--loombus-surface)] px-3 text-sm text-[var(--loombus-text)] outline-none focus:border-[var(--loombus-gold)]">
              ...
              <article key={publication.id} className="group relative min-w-0">
                ...
                <details className="absolute right-1 top-1 z-10">
                  ...
                  <div className="absolute right-0 mt-1 w-48 overflow-hidden rounded-xl border border-[var(--loombus-border)] bg-[var(--loombus-surface)] p-1.5 shadow-xl">
                    <Link href={`/library/publication/${publication.id}`} className="flex min-h-9 items-center gap-2 rounded-lg px-3 text-xs font-medium hover:bg-[var(--loombus-surface-muted)]"><BookOpen className="h-3.5 w-3.5" aria-hidden="true" />View details</Link>
      ...
      {!loading && !error && !rows.length ? (
        <div className="rounded-2xl border border-dashed border-[var(--loombus-border)] p-8 text-center"><h3 className="text-sm font-semibold">No published matches</h3><p className="mx-auto mt-2 max-w-md text-sm leading-6 text-[var(--loombus-text-muted)]">Try another title, author, publisher, ISBN, series, subject, audience, type, or topic.</p></div>
      ) : null}
