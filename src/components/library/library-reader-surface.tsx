...
type SearchResult = { id: string; pageIndex: number; title: string; snippet: string };
...
function paginateSections(sections: ReaderSection[], capacity: number): ReaderPage[] {
  ...
  sections.forEach((section, sectionIndex) => {
    ...
    while (start < text.length) {
      ...
      pages.push({
        id: `${section.section_key}:${part}:${start}`,
        sectionIndex,
...
function renderPageHighlights(page: ReaderPage, highlights: Highlight[], sectionSha: string | null) {
  const ranges = highlights
    ...
    .map((row) => ({ start: Math.max(page.start, row.start_offset as number) - page.start, end: Math.min(page.end, row.end_offset as number) - page.start }))
    .sort((a, b) => a.start - b.start || a.end - b.end);
  if (!ranges.length) return page.text;
  const parts = [];
  let cursor = 0;
  ranges.forEach((range, index) => {
    if (range.start > cursor) parts.push(<span key={`t-${index}-${cursor}`}>{page.text.slice(cursor, range.start)}</span>);
    parts.push(<mark key={`h-${index}-${range.start}`} className="rounded-[2px] bg-[#f3cf66]/70 text-inherit">{page.text.slice(range.start, range.end)}</mark>);
    cursor = Math.max(cursor, range.end);
  });
  if (cursor < page.text.length) parts.push(<span key={`tail-${cursor}`}>{page.text.slice(cursor)}</span>);
  return parts;
}
...
function buildSearchResults(pages: ReaderPage[], query: string): SearchResult[] {
  ...
  pages.forEach((page, pageIndex) => {
    ...
    results.push({ id: `${page.id}:${index}`, pageIndex, title: page.title, snippet: `${start > 0 ? "…" : ""}${page.text.slice(start, end).replace(/\s+/g, " ").trim()}${end < page.text.length ? "…" : ""}` });
  });
  return results;
}
...
export function LibraryReaderSurface({ publicationId }: { publicationId: string }) {
  ...
  useEffect(() => setPreferences(readLibraryReaderPreferences()), []);
  ...
  useEffect(() => {
    function updateCapacity() {
      ...
      const height = Math.max(460, window.innerHeight - (mobile ? 170 : 160));
      const linePx = preferences.fontSize * preferences.lineHeight;
      const charsPerLine = Math.max(26, Math.floor((mobile ? window.innerWidth - 64 : width - 96) / (preferences.fontSize * (preferences.font === "serif" ? 0.53 : 0.56))));
      const lines = Math.max(12, Math.floor(height / linePx));
      setPageCapacity(Math.max(520, Math.floor(charsPerLine * lines * 0.92)));
    }
    updateCapacity();
  ...
  const loadReader = useCallback(async () => {
    ...
    if (sectionResult.error || bookmarkResult.error) {
      setError("Unable to load this publication's reading state.");
      setLoading(false);
      return;
    }
    const nextSections = (sectionResult.data ?? []) as ReaderSection[];
    setSections(nextSections);
    setProgress((progressResult.data as Progress | null) ?? { locator: nextSections[0]?.section_key ?? null, progress_percent: 0 });
  ...
  useEffect(() => {
    if (!pages.length || !progress.locator) return;
    const index = pages.findIndex((page) => page.locator === progress.locator);
    if (index >= 0) setPageIndex(index);
  }, [pages.length]); // intentionally restore only after repagination/load
  ...
  async function persistPage(nextIndex: number) {
    ...
    const { error: saveError } = await supabase.from("library_reading_progress").upsert({ user_id: userId, publication_id: publicationId, locator: page.locator, progress_percent: nextProgress, last_read_at: new Date().toISOString(), updated_at: new Date().toISOString() }, { onConflict: "user_id,publication_id" });
    if (saveError) setError("Unable to save your reading position.");
  }

  function turn(delta: number) {
    const step = spreadCount;
    void persistPage(pageIndex + delta * step);
  }
  ...
  useEffect(() => {
    function onKeyDown(event: KeyboardEvent) {
      ...
      if (event.key === "Escape") { setPanel(null); setMobileMenuOpen(false); }
    }
  ...
  function captureSelection(page: ReaderPage, container: HTMLDivElement) {
    ...
    if (!browserSelection || browserSelection.rangeCount !== 1 || browserSelection.isCollapsed) { setSelection(null); return; }
    ...
    if (section.content_text.slice(startOffset, endOffset) !== text) {
      setError("Select text within one displayed page to use reading tools.");
      return;
    }
  ...
  async function saveSelectionNote() {
    if (!userId || !selection || !noteDraft.trim() || !currentPage) return;
    setSaving(true);
    let highlight = highlights.find((row) => row.locator === currentPage.locator && row.start_offset === selection.startOffset && row.end_offset === selection.endOffset) ?? null;
    if (!highlight) highlight = await saveHighlight();
    if (!highlight) { setSaving(false); return; }
    const { data, error: saveError } = await supabase.from("library_notes").insert({ user_id: userId, publication_id: publicationId, highlight_id: highlight.id, locator: currentPage.locator, body: noteDraft.trim() }).select("id, highlight_id, locator, body, created_at").single();
    if (saveError || !data) setError("Unable to save this note.");
    else { setNotes((rows) => [data as Note, ...rows]); setNoteDraft(""); setNoteForSelection(false); setSelection(null); window.getSelection()?.removeAllRanges(); }
    setSaving(false);
  }
  ...
  function pageForLocator(locator: string) {
    return pages.findIndex((page) => page.locator === locator);
  }
  ...
  return (
    <main
      ...
      onClick={(event) => {
        if (window.innerWidth >= 768 && event.target === event.currentTarget) setChromeVisible((value) => !value);
      }}
    ...
    >
      <header className={`absolute inset-x-0 top-0 z-40 flex h-16 items-center justify-between px-4 transition-opacity sm:px-6 ${chromeVisible ? "opacity-100" : "pointer-events-none opacity-0"}`}>
        <div className="flex items-center gap-1">
          <Link href="/library" className="grid h-10 w-10 place-items-center rounded-full hover:bg-black/5 dark:hover:bg-white/10" aria-label="Back to Library"><ArrowLeft className="h-5 w-5" /></Link>
        ...
        <div className="pointer-events-none absolute left-1/2 max-w-[45vw] -translate-x-1/2 truncate text-sm font-semibold opacity-75">{publication?.title}</div>
        <div className="flex items-center gap-1">
      ...
      {error ? <div className="absolute left-1/2 top-20 z-50 w-[min(36rem,calc(100vw-2rem))] -translate-x-1/2 rounded-2xl border border-black/10 bg-white/90 p-3 text-sm text-black shadow-xl backdrop-blur-xl">{error}</div> : null}
      ...
      <div className="absolute inset-0 flex items-center justify-center px-4 pb-20 pt-20 sm:px-8 md:pb-14 md:pt-16">
        ...
        <div className={`grid h-full w-full gap-4 ${spreadCount === 2 ? "max-w-[1180px] grid-cols-2" : "max-w-[min(760px,94vw)] grid-cols-1"}`}>
          {visiblePages.map((page, visibleIndex) => (
            <article key={page.id} className={`relative h-full min-h-0 overflow-hidden rounded-[1.35rem] px-7 py-9 shadow-[0_14px_45px_rgba(0,0,0,0.08)] sm:px-10 md:rounded-[1rem] md:px-12 md:py-10 ${preferences.theme === "night" ? "bg-[#090909]" : preferences.theme === "quiet" ? "bg-[#f3f0e8]" : preferences.theme === "loombus" ? "bg-[#fff9df]" : "bg-white"}`}>
              <div className="mb-5 hidden text-[11px] font-semibold uppercase tracking-[0.14em] opacity-45 md:block">{page.title}</div>
              <div
                ref={visibleIndex === 0 ? pageRef : undefined}
                ...
                className="h-[calc(100%-2rem)] whitespace-pre-line selection:bg-[#f3cf66]/70"
                style={{ fontFamily: pageFont, fontSize: preferences.fontSize, lineHeight: preferences.lineHeight }}
              >
                {renderPageHighlights(page, highlights, sectionHashes[page.locator] ?? null)}
              </div>
              <div className="absolute bottom-4 left-1/2 -translate-x-1/2 text-[10px] opacity-40 md:block hidden">{pageIndex + visibleIndex + 1}</div>
            </article>
          ))}
        </div>
      ...
      {selection ? (
        <div className="absolute bottom-24 left-1/2 z-[75] w-[min(38rem,calc(100vw-2rem))] -translate-x-1/2 rounded-2xl border border-black/10 bg-[#242424]/95 p-2 text-white shadow-2xl backdrop-blur-xl md:bottom-8">
          {!noteForSelection ? (
            <div className="grid grid-cols-2 gap-1 sm:grid-cols-4">
              <button type="button" onClick={() => void saveHighlight().then((saved) => { if (saved) { setSelection(null); window.getSelection()?.removeAllRanges(); } })} className="inline-flex min-h-10 items-center justify-center gap-2 rounded-xl px-3 text-sm hover:bg-white/10"><Highlighter className="h-4 w-4" />Highlight</button>
            ...
            <div className="flex gap-2"><input autoFocus value={noteDraft} onChange={(event) => setNoteDraft(event.target.value)} placeholder="Add a private note…" className="min-w-0 flex-1 rounded-xl bg-white/10 px-3 py-2 text-sm outline-none" /><button type="button" onClick={() => void saveSelectionNote()} disabled={!noteDraft.trim() || saving} className="rounded-xl bg-[var(--loombus-gold)] px-4 text-sm font-semibold text-black disabled:opacity-40">Save</button><button type="button" onClick={() => setNoteForSelection(false)} className="grid h-10 w-10 place-items-center"><X className="h-4 w-4" /></button></div>
          )}
        </div>
      ) : null}

      <div className={`absolute inset-x-0 bottom-4 z-40 flex items-center justify-center transition-opacity md:hidden ${chromeVisible ? "opacity-100" : "pointer-events-none opacity-0"}`}>
        <span className="text-sm font-semibold opacity-55">{progressDisplay}</span>
      </div>
      <button type="button" onClick={() => setMobileMenuOpen((value) => !value)} className={`absolute bottom-6 right-5 z-50 grid h-14 w-14 place-items-center rounded-full border border-white/10 bg-[#242424]/95 text-white shadow-xl transition-opacity md:hidden ${chromeVisible ? "opacity-100" : "opacity-0"}`} aria-label="Reader controls"><Menu className="h-6 w-6" /></button>
      ...
      {mobileMenuOpen ? (
        <div className="absolute inset-0 z-[60] bg-black/15 md:hidden" onClick={() => setMobileMenuOpen(false)}>
          <div className="absolute inset-x-3 bottom-20 space-y-2" onClick={(event) => event.stopPropagation()}>
            <button type="button" onClick={() => { setPanel("contents"); setMobileMenuOpen(false); }} className="flex min-h-16 w-full items-center justify-between rounded-[2rem] bg-[#3a3a3c]/95 px-6 text-left text-xl text-white"><span>Contents · {progressPercent}%</span><List className="h-7 w-7" /></button>
            <button type="button" onClick={() => { setPanel("search"); setMobileMenuOpen(false); }} className="flex min-h-16 w-full items-center justify-between rounded-[2rem] bg-[#3a3a3c]/95 px-6 text-left text-xl text-white"><span>Search Book</span><Search className="h-7 w-7" /></button>
            <button type="button" onClick={() => { setPanel("appearance"); setMobileMenuOpen(false); }} className="flex min-h-16 w-full items-center justify-between rounded-[2rem] bg-[#3a3a3c]/95 px-6 text-left text-xl text-white"><span>Themes & Settings</span><span className="text-2xl">AA</span></button>
            <div className="grid grid-cols-3 gap-2">
              <button type="button" onClick={() => { setPanel("annotations"); setMobileMenuOpen(false); }} className="grid min-h-16 place-items-center rounded-[2rem] bg-[#3a3a3c]/95 text-white"><NotebookPen className="h-7 w-7" /></button>
              {historyPage !== null ? <button type="button" onClick={() => { const target = historyPage; setHistoryPage(null); setMobileMenuOpen(false); void persistPage(target); }} className="grid min-h-16 place-items-center rounded-[2rem] bg-[#3a3a3c]/95 text-white"><Undo2 className="h-7 w-7" /></button> : <div className="rounded-[2rem] bg-[#3a3a3c]/75" />}
              <button type="button" onClick={() => void toggleBookmark()} className="grid min-h-16 place-items-center rounded-[2rem] bg-[#3a3a3c]/95 text-white">{currentBookmark ? <BookmarkCheck className="h-7 w-7" /> : <Bookmark className="h-7 w-7" />}</button>
            </div>
          </div>
        </div>
      ) : null}
      ...
      {panel ? (
        <aside className="absolute inset-y-0 right-0 z-[70] w-full overflow-y-auto border-l border-black/10 bg-white/96 p-5 text-[#211f1a] shadow-2xl backdrop-blur-2xl sm:w-[24rem] md:top-0">
