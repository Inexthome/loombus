...
export function LibraryReaderSurface({ publicationId }: { publicationId: string }) {
  ...
  const loadReader = useCallback(async () => {
    ...
    const savedIndex = savedProgress?.locator
      ? normalizedSections.findIndex((section) => section.section_key === savedProgress.locator)
      : -1;
    if (savedProgress && savedIndex >= 0) {
      setProgress(savedProgress);
    } else {
      setProgress({ locator: normalizedSections[0].section_key, progress_percent: progressPercent(0, normalizedSections.length) });
    }
  ...
  async function moveTo(index: number) {
    ...
    const { error: saveError } = await supabase.from("library_reading_progress").upsert({
      user_id: userId,
  ...
  async function saveHighlight() {
    ...
    }).select("id, locator, selected_text, created_at").single();
  ...
  async function saveNote() {
    ...
    setSaving(true);
    const { data, error: saveError } = await supabase.from("library_notes").insert({
      user_id: userId,
      publication_id: publicationId,
      locator: currentSection.section_key,
      body: noteDraft.trim(),
    }).select("id, highlight_id, locator, body, created_at").single();
          ...
          <div className="mt-6 grid gap-6 lg:grid-cols-[minmax(0,1fr)_19rem]">
            <article className="rounded-[2rem] border border-[var(--loombus-border)] bg-[var(--loombus-reader-paper,var(--loombus-surface))] px-6 py-8 sm:px-10 sm:py-12">
              <div className="mx-auto max-w-3xl">
                <p className="text-xs font-bold uppercase tracking-[0.2em] text-[var(--loombus-gold)]">{currentSection.title ?? `Section ${currentSection.ordinal + 1}`}</p>
                ...
                <div onMouseUp={captureSelection} onTouchEnd={captureSelection} className="mt-9 whitespace-pre-line leading-[1.9]" style={{ fontSize }}>{currentSection.content_text}</div>
            ...
            <aside className="space-y-4">
              <section className="rounded-[1.5rem] border border-[var(--loombus-border)] bg-[var(--loombus-surface)] p-5"><div className="flex items-center gap-2"><BookOpen className="h-4 w-4 text-[var(--loombus-gold)]" /><h2 className="font-semibold">Reading position</h2></div><div className="mt-4 space-y-1">{sections.map((section, index) => <button key={section.section_key} onClick={() => void moveTo(index)} className={`w-full rounded-xl px-3 py-2 text-left text-sm ${section.section_key === currentSection.section_key ? "bg-[var(--loombus-gold-surface)] font-semibold" : "text-[var(--loombus-text-muted)]"}`}>{section.title ?? `Section ${section.ordinal + 1}`}</button>)}</div></section>
