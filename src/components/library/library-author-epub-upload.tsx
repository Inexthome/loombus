...
export function LibraryAuthorEpubUpload({ publicationId, editable, published, onReadyChange }: Props) {
  ...
  const loadSource = useCallback(async () => {
    ...
    const result = await supabase
      ...
      .select("id, publication_id, storage_bucket, storage_path, byte_size, ingestion_status, ingestion_error, updated_at")
      .eq("publication_id", publicationId)
      .maybeSingle();

    if (result.error) {
      setSource(null);
      setError("Unable to load EPUB status.");
      setLoading(false);
      return;
    }
  ...
  async function uploadAndProcess() {
    ...
    try {
      ...
      if (!Number.isSafeInteger(file.size) || file.size < 1 || file.size > MAX_EPUB_BYTES) {
        throw new Error("EPUB files must be 50 MiB or smaller.");
      }
      if (file.type && file.type !== "application/epub+zip" && file.type !== "application/octet-stream") {
        throw new Error("Choose a valid EPUB file.");
      }

      const sha256 = await sha256File(file);
      const prepareResult = await supabase.rpc("prepare_library_author_epub_source", {
