...
import { ArrowLeft, BookCheck, BookOpen, Bookmark, CalendarDays, Clock3, Languages, LibraryBig, Loader2, ScrollText, UserRound } from "lucide-react";
...
type LifecycleRow = { state: LifecycleState; finished_at: string | null; updated_at: string };
...
export function LibraryPublicationDetail({ publicationId }: { publicationId: string }) {
  ...
  const loadDetail = useCallback(async () => {
    ...
    if (!currentUser) { setUserId(null); setSaved(false); setProgress(null); setLifecycle(null); setLoading(false); return; }
    setUserId(currentUser.id);

    const [savedResult, progressResult, lifecycleResult, profileResult] = await Promise.all([
      ...
      supabase.from("library_reading_lifecycle").select("state,finished_at,updated_at").eq("user_id", currentUser.id).eq("publication_id", publicationId).maybeSingle(),
      supabase.rpc("get_library_published_author_profile", { p_publication_id: publicationId }),
    ]);
    if (savedResult.error || progressResult.error || lifecycleResult.error) setError("Publication loaded, but your private Library state could not be loaded.");
    setSaved(Boolean(savedResult.data));
    setProgress((progressResult.data ?? null) as ProgressRow | null);
    setLifecycle((lifecycleResult.data ?? null) as LifecycleRow | null);
  ...
  async function setLifecycleState(state: LifecycleState) {
    if (!publication || statusSaving) return;
    if (!userId) { window.location.assign(`/login?next=/library/publication/${publication.id}`); return; }
    setStatusSaving(true); setError(null);
    if (state === "want_to_read" && !saved) {
      const savedResult = await supabase.from("library_member_items").insert({ user_id: userId, publication_id: publication.id });
      if (savedResult.error) { setError("Unable to add this publication to My Library."); setStatusSaving(false); return; }
      setSaved(true);
    }
    const now = new Date().toISOString();
    const { data, error: lifecycleError } = await supabase.from("library_reading_lifecycle").upsert({ user_id: userId, publication_id: publication.id, state, finished_at: state === "finished" ? now : null, updated_at: now }, { onConflict: "user_id,publication_id" }).select("state,finished_at,updated_at").single();
    if (lifecycleError || !data) setError("Unable to update this publication's reading status."); else setLifecycle(data as LifecycleRow);
  ...
  async function toggleSaved() {
    ...
    if (saved) {
      ...
      if (result.error) setError("Unable to remove this publication from My Library.");
      else {
        setSaved(false);
        if (lifecycle?.state === "want_to_read") {
          const lifecycleDelete = await supabase.from("library_reading_lifecycle").delete().eq("user_id", userId).eq("publication_id", publication.id).eq("state", "want_to_read");
          if (!lifecycleDelete.error) setLifecycle(null);
        }
      }
    } else {
      const result = await supabase.from("library_member_items").insert({ user_id: userId, publication_id: publication.id });
      if (result.error) setError("Unable to add this publication to My Library.");
      else {
        setSaved(true);
        if (!lifecycle) {
          const now = new Date().toISOString();
          const lifecycleResult = await supabase.from("library_reading_lifecycle").insert({ user_id: userId, publication_id: publication.id, state: "want_to_read", finished_at: null, updated_at: now }).select("state,finished_at,updated_at").single();
          if (!lifecycleResult.error && lifecycleResult.data) setLifecycle(lifecycleResult.data as LifecycleRow);
        }
      }
    }
    setSaving(false);
  ...
  const lifecycleLabel = lifecycle?.state === "finished" ? "Finished" : lifecycle?.state === "reading" ? "Reading" : lifecycle?.state === "want_to_read" ? "Want to Read" : null;
  ...
  return (
    <main className="min-h-screen bg-[var(--loombus-page-bg)] px-4 pb-24 pt-6 text-[var(--loombus-text)] sm:px-6 md:pt-24 lg:px-8">
      <div className="mx-auto w-full max-w-6xl">
        ...
        <section className="mt-5 grid gap-7 border-b border-[var(--loombus-border)] pb-8 lg:grid-cols-[220px_minmax(0,1fr)] lg:gap-10">
          <div className="mx-auto w-full max-w-[220px] lg:mx-0"><div className="grid aspect-[2/3] w-full place-items-center overflow-hidden rounded-xl border border-[var(--loombus-border)] bg-[var(--loombus-gold-surface)] text-[var(--loombus-gold)] shadow-sm"><LibraryCoverImage storagePath={publication.cover_url} alt={`${publication.title} cover`} fallbackClassName="h-10 w-10" /></div></div>
          <div className="min-w-0">
            <div className="flex flex-wrap items-center gap-2"><span className="rounded-full border border-[var(--loombus-border)] px-3 py-1 text-[10px] font-bold uppercase tracking-[0.14em] text-[var(--loombus-text-muted)]">{publication.publication_type}</span>{lifecycleLabel ? <span className="rounded-full bg-[var(--loombus-gold-surface)] px-3 py-1 text-xs font-semibold text-[var(--loombus-gold)]">{lifecycleLabel}</span> : null}{publication.edition_label ? <span className="rounded-full bg-[var(--loombus-surface-strong)] px-3 py-1 text-xs">{publication.edition_label}</span> : null}</div>
            ...
            {userId ? <div className="mt-3 flex flex-wrap gap-2">{lifecycle?.state !== "want_to_read" ? <button type="button" disabled={statusSaving} onClick={() => void setLifecycleState("want_to_read")} className="inline-flex min-h-9 items-center gap-2 rounded-full border border-[var(--loombus-border)] px-4 text-xs font-semibold hover:border-[var(--loombus-gold)] disabled:opacity-50"><Clock3 className="h-3.5 w-3.5" />Want to Read</button> : null}{lifecycle?.state === "finished" ? <button type="button" disabled={statusSaving} onClick={() => void setLifecycleState("reading")} className="inline-flex min-h-9 items-center gap-2 rounded-full border border-[var(--loombus-border)] px-4 text-xs font-semibold hover:border-[var(--loombus-gold)] disabled:opacity-50"><BookOpen className="h-3.5 w-3.5" />Still Reading</button> : <button type="button" disabled={statusSaving} onClick={() => void setLifecycleState("finished")} className="inline-flex min-h-9 items-center gap-2 rounded-full border border-[var(--loombus-border)] px-4 text-xs font-semibold hover:border-[var(--loombus-gold)] disabled:opacity-50"><BookCheck className="h-3.5 w-3.5" />Mark as Finished</button>}</div> : null}
            {hasProgress ? <div className="mt-6 max-w-xl"><div className="flex items-center justify-between gap-3 text-xs"><span className="font-semibold">Reading progress</span><span className="font-semibold text-[var(--loombus-gold)]">{Math.round(progressPercent)}%</span></div><div className="mt-3 h-1.5 overflow-hidden rounded-full bg-[var(--loombus-surface-muted)]"><div className="h-full rounded-full bg-[var(--loombus-gold)]" style={{ width: `${progressPercent}%` }} /></div></div> : null}
          </div>
        </section>
