...
export function RoomGuestsNavigationBridge() {
  ...
  const href = `/rooms/${encodeURIComponent(roomId)}/guests`;
  const active = pathname === href || pathname.startsWith(`${href}/`);
  return createPortal(
    <Link href={href} aria-current={active ? "page" : undefined} className="rooms-phase1-nav-link" title="Guests">
      <UserRoundCheck aria-hidden="true" /><span>Guests</span>
    </Link>, target
  );
}
