...
export function DiscussionsCreateComposerBridge() {
  ...
  useEffect(() => {
    ...
    function locateSearchBar() {
      ...
      tabRow = controlParent ? (Array.from(controlParent.children).find((candidate) => {
        const labels = Array.from(candidate.querySelectorAll("button")).map((button) => button.textContent?.trim() ?? "");
        return FILTER_LABELS.every((label) => labels.includes(label));
      }) as HTMLElement | undefined) ?? null : null;
  ...
  return (
    <>
      {slot ? createPortal(<CreateTrigger buttonRef={triggerRef} onOpen={() => setOpen(true)} />, slot) : null}
      {filterSlot ? createPortal(<FilterControl open={filterOpen} selected={selectedFilter} onToggle={() => setFilterOpen((current) => !current)} onSelect={selectFilter} controlRef={filterControlRef} />, filterSlot) : null}
