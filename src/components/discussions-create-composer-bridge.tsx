...
function CreateTrigger({ onOpen, buttonRef }: { onOpen: () => void; buttonRef: RefObject<HTMLButtonElement | null> }) {
  return (
    <button ref={buttonRef} type="button" onClick={onOpen} className="inline-flex size-11 items-center justify-center bg-transparent text-[color:var(--loombus-gold)] focus-visible:outline-none" aria-label="Start a discussion" aria-haspopup="dialog" title="Start a discussion">
...
function FilterControl({ open, selected, onToggle, onSelect, controlRef }: { open: boolean; selected: FilterLabel; onToggle: () => void; onSelect: (label: FilterLabel) => void; controlRef: RefObject<HTMLDivElement | null> }) {
  return (
    <div ref={controlRef} className="relative">
      <button type="button" onClick={onToggle} className="inline-flex size-11 items-center justify-center bg-transparent text-[color:var(--loombus-text-muted)] focus-visible:outline-none" aria-label="Filter discussions" aria-haspopup="menu" aria-expanded={open} title="Filter discussions">
        <SlidersHorizontal aria-hidden="true" className="size-5" strokeWidth={2.1} />
      </button>
      {open ? (
        <div role="menu" aria-label="Discussion filters" className="absolute right-0 top-[calc(100%+0.5rem)] z-50 w-[min(17rem,calc(100vw-2rem))] overflow-hidden rounded-2xl border border-[color:var(--loombus-border)] bg-[color:var(--loombus-surface)] p-2 shadow-2xl shadow-black/15" data-discussions-filter-menu>
          {FILTER_LABELS.map((label) => {
            const isSelected = selected === label;
            return (
              <button key={label} type="button" role="menuitemradio" aria-checked={isSelected} onClick={() => onSelect(label)} className={`flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-left text-sm font-semibold transition ${isSelected ? "bg-amber-50 text-[#8a6519] dark:bg-amber-400/10 dark:text-[#CBAB5B]" : "text-[color:var(--loombus-text)] hover:bg-[color:var(--loombus-surface-muted)]"}`}>
                <span className="sr-only">Filter: </span><span className="min-w-0 flex-1">{label}</span>{isSelected ? <Check aria-hidden="true" className="h-4 w-4" /> : null}
              </button>
            );
          })}
        </div>
...
function CreateModal({ onClose }: { onClose: () => void }) {
  ...
  useEffect(() => {
    ...
    function handleKeyDown(event: KeyboardEvent) {
      if (event.key === "Escape" && !document.querySelector('button[aria-label="Close composer options"]')) onClose();
    }
    window.addEventListener("keydown", handleKeyDown);
    return () => {
      window.removeEventListener("keydown", handleKeyDown);
      document.body.style.overflow = previousOverflow;
      if (previousComposerState === undefined) delete document.body.dataset.discussionsCreateOpen;
      else document.body.dataset.discussionsCreateOpen = previousComposerState;
    };
  }, [onClose]);

  return createPortal(
    <div className="discussions-create-modal-backdrop" data-discussions-create-modal>
      <section ref={panelRef} className="discussions-create-modal-panel" role="dialog" aria-modal="true" aria-label="Start a discussion" tabIndex={-1}>
...
export function DiscussionsCreateComposerBridge() {
  ...
  useEffect(() => {
    ...
    function handlePointerDown(event: PointerEvent) {
      ...
      if (target && !filterControlRef.current?.contains(target)) setFilterOpen(false);
    }
  ...
  useEffect(() => {
    if (pathname !== "/discussions") {
      setSlot(null); setFilterSlot(null); setOpen(false); setFilterOpen(false); setSelectedFilter("All"); hiddenTabRowRef.current = null; hiddenResetButtonRef.current = null; return;
    }
    ...
    function locateSearchBar() {
      ...
      const heading = Array.from(feedRoot?.querySelectorAll("h1") ?? []).find((candidate) => candidate.textContent?.trim() === "Discussions");
      headingBlock = heading?.parentElement ?? null;
      resetButton = controlsRow.querySelector<HTMLButtonElement>('button[aria-label="Reset discussion filters"]');
      ...
      searchLabel.hidden = true; searchLabel.setAttribute("aria-hidden", "true");
      if (headingBlock) { headingBlock.hidden = true; headingBlock.setAttribute("aria-hidden", "true"); }
      tabRow.hidden = true; tabRow.setAttribute("aria-hidden", "true"); resetButton.hidden = true; resetButton.setAttribute("aria-hidden", "true");
      hiddenTabRowRef.current = tabRow; hiddenResetButtonRef.current = resetButton;
      composerMount = document.createElement("div"); composerMount.dataset.discussionsCreateTriggerSlot = "true"; composerMount.className = "shrink-0"; searchLabel.insertAdjacentElement("afterend", composerMount);
      filterMount = document.createElement("div"); filterMount.dataset.discussionsFilterSlot = "true"; filterMount.className = "shrink-0"; controlsRow.append(filterMount);
      setSlot(composerMount); setFilterSlot(filterMount);
    }
    locateSearchBar();
    return () => {
      cancelled = true; window.clearTimeout(timer); setSlot(null); setFilterSlot(null); setOpen(false); setFilterOpen(false); setSelectedFilter("All"); hiddenTabRowRef.current = null; hiddenResetButtonRef.current = null; composerMount?.remove(); filterMount?.remove();
      if (searchLabel) { searchLabel.hidden = false; searchLabel.removeAttribute("aria-hidden"); }
      if (headingBlock) { headingBlock.hidden = false; headingBlock.removeAttribute("aria-hidden"); }
      if (tabRow) { tabRow.hidden = false; tabRow.removeAttribute("aria-hidden"); }
      if (resetButton) { resetButton.hidden = false; resetButton.removeAttribute("aria-hidden"); }
    };
  }, [pathname]);

  function closeModal() { setOpen(false); window.setTimeout(() => triggerRef.current?.focus(), 0); }
  function selectFilter(label: FilterLabel) {
    if (label === "All") hiddenResetButtonRef.current?.click();
    else Array.from(hiddenTabRowRef.current?.querySelectorAll<HTMLButtonElement>("button") ?? []).find((button) => button.textContent?.trim() === label)?.click();
    setSelectedFilter(label); setFilterOpen(false);
  }
