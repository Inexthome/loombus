...
function accuracyLabel(value: FloorCredibilityRow["accuracy_pct"]) {
...
export default function TheFloorLeaderboardPage() {
  ...
  const load = useCallback(async () => {
    const { data, error } = await supabase
      ...
      .select(
        "member_id, username, full_name, pending_calls, resolved_calls, correct_calls, incorrect_calls, partial_calls, accuracy_pct, last_resolved_at"
      )
      .order("accuracy_pct", { ascending: false, nullsFirst: false })
      .order("resolved_calls", { ascending: false })
      .limit(50);
    if (!error && data) {
      setRows(data as FloorCredibilityRow[]);
    }
    setLoading(false);
  }, []);
  ...
  if (loading) {
    return (
      <LoombusLoadingScreen
        ...
        message="Computing track records from resolved calls."
      />
    );
  }
      ...
      <div className="mx-auto flex max-w-3xl flex-col gap-5">
        <header className="rounded-[1.75rem] border border-[var(--loombus-border)] bg-[var(--loombus-surface)] p-5 shadow-xl shadow-black/10">
          ...
          <div className="mt-3 flex items-center gap-3">
            <Trophy className="size-7 text-[var(--loombus-gold)]" aria-hidden="true" />
            <div>
              <h1 className="text-2xl font-black sm:text-3xl">Track record leaderboard</h1>
              <p className="mt-1 max-w-xl text-sm leading-6 text-[var(--loombus-text-muted)]">
                Computed live from resolved falsifiable calls. Never a house rating &mdash; this
                ranks members on their own predictions, not our opinion of any stock.
              </p>
            </div>
          </div>
        </header>
        ...
        {rows.length === 0 ? (
          <div className="flex flex-col items-center gap-3 rounded-[1.75rem] border border-dashed border-[var(--loombus-border)] bg-[var(--loombus-surface)] p-10 text-center">
            ...
            <p className="text-sm font-bold text-[var(--loombus-text-muted)]">
              No track records yet. Once falsifiable calls are resolved, member track records will
              appear here.
            </p>
          </div>
        ) : (
          <ol className="flex flex-col gap-3">
            {rows.map((row, index) => (
              <li
                key={row.member_id}
                className="flex items-center gap-4 rounded-3xl border border-[var(--loombus-border)] bg-[var(--loombus-surface)] p-4 shadow-xl shadow-black/10"
              >
                <span
                  className={`w-8 shrink-0 text-center text-lg font-black ${
                    RANK_STYLES[index] ?? "text-[var(--loombus-text-subtle)]"
                  }`}
                >
                  #{index + 1}
                </span>
                <div className="min-w-0 flex-1">
                  <p className="truncate text-sm font-black text-[var(--loombus-text)]">
                    {floorDisplayName(row.full_name, row.username)}
                  </p>
                  <div className="mt-1 flex flex-wrap gap-x-3 gap-y-1 text-xs font-bold text-[var(--loombus-text-muted)]">
                    ...
                    {row.partial_calls > 0 ? (
                      <span className="text-amber-400">{row.partial_calls} partial</span>
                    ) : null}
                    {row.pending_calls > 0 ? <span>{row.pending_calls} pending</span> : null}
                  </div>
                </div>
                <span className="shrink-0 rounded-full bg-[var(--loombus-gold-surface)] px-3 py-1.5 text-sm font-black text-[var(--loombus-gold)]">
                  {accuracyLabel(row.accuracy_pct)}
                </span>
              </li>
            ))}
          </ol>
