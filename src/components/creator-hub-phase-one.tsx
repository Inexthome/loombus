...
export function CreatorHubPhaseOne() {
  ...
  useEffect(() => {
    ...
    async function loadCreatorHub() {
      ...
      const entitled =
        Boolean(loadedProfile?.is_admin) ||
      ...
      if (!cancelled) {
        setMetrics({
          discussions: metricValue(discussionResult),
          followers: metricValue(followerResult),
          profileViews: metricValue(viewResult),
        });
      }
  ...
  async function saveCreatorLinks() {
    ...
    if (!canUseCreatorTools) {
      setMessage("Creator links require Premium Plus access.");
      return;
    }

    if (!validOptionalUrl(cleanWebsiteUrl) || !validOptionalUrl(cleanSupportUrl)) {
      setMessage("Website and support URLs must begin with http:// or https://.");
      return;
    }
    ...
    try {
      const response = await fetch("/api/profile/public", {
        ...
        body: JSON.stringify({
          ...
          creatorWebsiteUrl: cleanWebsiteUrl,
          creatorSupportUrl: cleanSupportUrl,
          creatorSupportLabel: cleanSupportLabel,
        }),
      });
      const result = await response.json().catch(() => ({}));

      if (!response.ok) {
        setMessage(result.error ?? "Unable to save creator links.");
        return;
      }
      ...
      setProfile((current) =>
        current
          ? {
              ...current,
              creator_website_url: cleanWebsiteUrl,
              creator_support_url: cleanSupportUrl,
              creator_support_label: cleanSupportLabel,
            }
          : current
      );
  ...
  if (loading) {
    return (
      <section className="creator-hub-phase-one" aria-busy="true">
  ...
  const launchCards = [
    {
      title: "Publish",
      description: "Create a public discussion, research question, debate, or media post.",
      href: "/create",
    ...
    {
      title: "Rooms",
      description: "Build and manage the community spaces connected to your work.",
      href: "/rooms",
      action: "Manage Rooms",
      Icon: Users,
    },
    {
      title: "Storefront",
      description: "Manage Marketplace listings and make your offers discoverable.",
      href: "/marketplace",
    ...
    {
      title: "Services",
      description: "Publish services people can discover from your public profile.",
      href: "/services",
    ...
    {
      title: "Appointments",
      description: "Manage bookable time and service-based appointments.",
      href: "/appointments",
  ...
  return (
    <section className="creator-hub-phase-one">
      <header className="creator-hub-hero">
        <div>
          <p className="creator-hub-eyebrow">Creator Hub · Phase 1</p>
          <h2>Operate your public work from one place.</h2>
          <p>
            Connect your profile, content, Rooms, Marketplace presence, services, and appointments. Paid supporter tiers and Loombus payouts are not active in this phase.
          </p>
        </div>
        {publicProfilePath ? (
          <Link href={publicProfilePath} target="_blank" className="creator-hub-primary-link">
            View public profile <ExternalLink aria-hidden="true" />
          </Link>
        ) : (
          <Link href="/profile?section=public" className="creator-hub-primary-link">
            Complete public profile <ArrowUpRight aria-hidden="true" />
          </Link>
        )}
      </header>
      ...
      <div className="creator-hub-metrics" aria-label="Creator activity overview">
        <article>
          ...
          <span>{metrics.discussions}</span>
          <p>Published discussions</p>
        </article>
        <article>
        ...
        <article>
          ...
          <span>{metrics.profileViews}</span>
          <p>Profile views</p>
        </article>
      </div>
      ...
      <section className="creator-hub-section">
        <div className="creator-hub-section-heading">
          <div>
            <p className="creator-hub-eyebrow">Creator operations</p>
            <h3>Build with Loombus features</h3>
          </div>
        </div>
        <div className="creator-hub-launch-grid">
          {launchCards.map(({ title, description, href, action, Icon }) => (
            <article key={title}>
      ...
      <section className="creator-hub-section creator-hub-content-manager">
        <div className="creator-hub-section-heading">
          <div>
            <p className="creator-hub-eyebrow">Content manager</p>
            <h3>Recent discussions</h3>
          </div>
          <Link href="/my-discussions">View all</Link>
        </div>
        {recentDiscussions.length ? (
          <div className="creator-hub-discussion-list">
            {recentDiscussions.map((discussion) => (
              <Link key={discussion.id} href={`/discussions/${discussion.id}`}>
                <span>{discussion.title?.trim() || "Untitled discussion"}</span>
                <small>
          ...
          <div className="creator-hub-empty">
            <MessageSquareText aria-hidden="true" />
            <p>Your published discussions will appear here.</p>
            <Link href="/create">Create your first discussion</Link>
          </div>
        )}
      </section>
      ...
      <section className="creator-hub-section creator-hub-links-editor">
        <div className="creator-hub-section-heading">
          <div>
            <p className="creator-hub-eyebrow">Public creator links</p>
            <h3>Website and external support</h3>
            <p>
              These links appear on your public profile. Loombus does not process or owe payouts for payments completed through an external support service.
            </p>
          </div>
          {!canUseCreatorTools ? <Link href="/premium">Premium Plus</Link> : null}
        </div>

        <div className="creator-hub-link-fields">
          <label>
            <span><Globe2 aria-hidden="true" /> Creator website URL</span>
            <input
              type="url"
              value={websiteUrl}
          ...
          <label>
            <span><ExternalLink aria-hidden="true" /> External support URL</span>
            <input
              type="url"
              value={supportUrl}
        ...
        <div className="creator-hub-save-row">
          <button type="button" onClick={() => void saveCreatorLinks()} disabled={saving || !canUseCreatorTools}>
